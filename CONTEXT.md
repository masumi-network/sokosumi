...
**Channel tile**:
How a Channel identifies itself in the collapsed sidebar rail: the first letters of its name on a neutral square. A private, external, or matched Channel carries its kind as a small corner mark; a public one carries none. A Direct shows one 24px face instead — the first participant's, whoever else is in the room, because a Sidebar row's mark is one 24px slot and a stack of faces is not; the row's label lists everyone in that same order. Only a 1:1 face carries a presence dot: one dot among several people reads as the room's state rather than one person's. The expanded list keeps the plain kind glyph beside the name.
...
_Avoid_: A taller featured row, a card in the row list, a per-list leading box, a stack of faces or bots in the slot (it overflows the 24px box and runs under the label), a gap between rows (the rail has none), zero movement as the goal (the three exceptions still shift what is below them)
