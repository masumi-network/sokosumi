...
**Rail attention dot**:
The collapsed sidebar rail's single attention mark, top-right of a room's Channel tile or Direct avatar: primary for an unread User mention or direct, neutral for Room unread (including marked unread), none when read or Muted. Mention wins; never two dots and never a number. Derived from the same attention rules as the expanded row's bold and badge.
_Avoid_: Unread badge, count dot, notification dot, a second dot for mention plus unread, a dot on the expanded list
