...
**Channel tile**:
How a Channel identifies itself in the collapsed sidebar rail: the first letters of its name on a neutral square. A private, external, or matched Channel carries its kind as a small corner mark; a public one carries none. A Direct shows participant faces instead, 24px each: up to three overlapping when several people are in the room, the first alone on the rail, where a 32px square cannot hold the rest and the tooltip already names them. The expanded list keeps the plain kind glyph beside the name.
