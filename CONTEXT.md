...
**Pinned room**:
The current user's personal sidebar pin of a membership-visible room. Not shared. Product UI: Pin / Unpin; a Pinned room of any kind lists under the sidebar's Pinned section only, in the reader's own order, and activity never moves it. That order is the membership's `starredAt` ascending, which a reorder rewrites (`PUT /chats/rooms/starred`), so `starredAt` is a sort key, not the time of pinning. Distinct from a Pinned message.
