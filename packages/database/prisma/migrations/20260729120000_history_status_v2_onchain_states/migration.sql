...
-- completed or refund-pending.
--
-- Mirrors packages/database/src/helpers/job.ts computeJobStatus:
--   WITHDRAW_AUTHORIZED behaves like RESULT_SUBMITTED (withdrawal authorized
--   but not yet executed on-chain), REFUND_AUTHORIZED like REFUND_REQUESTED.
--
-- Backfill existing rows so the feed corrects itself instead of waiting for
-- each job's next update to fire the trigger.
...
BEGIN
  ...
  CASE source_job."onChainStatusText"
    WHEN 'FUNDS_LOCKED' THEN
      CASE latest_status
        ...
        ELSE
          ...
          RETURN 'processing';
      END CASE;
    WHEN 'RESULT_SUBMITTED', 'WITHDRAW_AUTHORIZED' THEN
    ...
    WHEN 'REFUND_REQUESTED', 'REFUND_AUTHORIZED' THEN RETURN 'refund_pending';
...
UPDATE "history"
SET "status" = compute_history_job_status("entityId")
WHERE "kind" = 'JOB'
  AND "entityId" IN (
    SELECT j."id"
    FROM "Job" j
    JOIN "jobPurchase" p ON p."jobId" = j."id"
    WHERE p."onChainStatus" IN ('WITHDRAW_AUTHORIZED', 'REFUND_AUTHORIZED')
  );
