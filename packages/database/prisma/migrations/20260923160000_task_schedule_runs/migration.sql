-- ADR 0040 expand step: Task Schedule Runs live in the existing ledger
...
-- A Task Schedule Run has no series Task.
...
-- One Run per schedule, rule epoch, and rule time, so a repeated
-- projection never plans (and releases) the same Run twice.
