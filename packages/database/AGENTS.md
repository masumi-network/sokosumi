...
- **Mirror hand-written SQL in the schema.** Declare every index, unique, or constraint name a migration creates in `schema.prisma` with the same `map:` name. Partial uniques use `@@unique([...], map: "...", where: { ... })`; only what Prisma cannot express (CHECKs, expression indexes) stays SQL-only, with a comment on the model. Check with `pnpm prisma:check-drift` against a database with every migration applied (a scratch Postgres after `pnpm prisma:migrate:deploy`): it prints the SQL the next `migrate dev` would generate and exits 2 on drift. Test Core runs it.
