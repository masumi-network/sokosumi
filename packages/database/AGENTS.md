...
- **One timestamp per folder, after the current tip.** Prisma orders folders by the 14-digit prefix (`YYYYMMDDHHMMSS`) and applies an older-dated pending folder without warning, so a stamp below `main`'s newest migration runs in a different order on fresh databases than in production. Take a fresh stamp above the tip instead of a round `YYYYMMDD120000` another PR may share. Applied folders keep their names forever, because renaming one re-runs its SQL; the historical collisions are allowlisted in `src/helpers/migration-prefix-uniqueness.ts`.
