...
- **Optional gate check.** In a fresh browser session without cookies, open `/history` and confirm redirect to `/signin?returnUrl=%2Fhistory`. Do not reuse `AGENT_BROWSER_SESSION_NAME` for this check (use another session name or clear state).
