...
- `agents-search-or-empty` covers search / filter miss (“No offers match your filters.”) or a blank body when no coworkers are returned.
...
- **Search miss (optional).** Type nonsense in the search field; expect **No offers match your filters.**
- **Empty path.** If the gallery body is blank (no coworker cards), capture screenshot + snapshot and stop — unmet coworker catalog precondition.
