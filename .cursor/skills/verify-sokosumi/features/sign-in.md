...
- **Submit.** Wait briefly after fill (~400ms), then `agent-browser press Enter` if still on `/signin`. Expect a soft navigation straight to Welcome `/` (or `returnUrl`); there is no callback hop for credential login. Do **not** `wait --load networkidle` here; Ably can hang that wait on chat-adjacent shells.
...
- Only OAuth and magic-link pass through `/auth/callback/signin`; credential and passkey soft-navigate in place. Harness `assert_authed_url` still allows `/auth/callback/` and waits for replace.
