...
- `turnstile=off` or `turnstile=test (always passes)` in doctor. `turnstile=live` (or `test` with “blocks” / “interactive challenge”) means the form renders a “confirm you are human” widget that an agent must **not** answer — and `fixture_auth=ok` can still report success, because the harness POSTs Core's sign-in endpoint with a dummy token and never sees the widget. `pnpm env:bootstrap` only fills a **commented-out** key; a live key already set in `.env` must be replaced with the 1x always-passes values in `apps/web/.env.example` / `apps/core/.env.example`.
