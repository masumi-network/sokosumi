...
- `turnstile=test` or `turnstile=off` in doctor. `turnstile=live` means a real Cloudflare key is set, so the form renders a “confirm you are human” widget that an agent must **not** answer — and `fixture_auth=ok` still reports success, because it calls Core's sign-in endpoint directly and never sees the captcha. Fix the env first (`pnpm env:bootstrap`, or the test keys documented in `apps/web/.env.example`).
