...
const organizationBillingFormSchema = z
  .object({
...
type OrganizationBillingFormData = z.infer<
  typeof organizationBillingFormSchema
>;
...
export default function OrganizationBillingForm({
  priceId,
  amountPerCredit,
  currency,
  organization,
}: OrganizationBillingFormProps) {
  ...
  const form = useForm<OrganizationBillingFormData>({
    resolver: zodResolver(organizationBillingFormSchema),
    defaultValues: {
      credits: undefined,
      coupon: undefined,
    },
  ...
  const onSubmit = async (data: OrganizationBillingFormData) => {
    if (!organization) {
    ...
    if (data.coupon && data.coupon.trim().length > 0) {
      result = await getFreeOrganizationCreditsWithCoupon(
        priceId,
        data.coupon.trim(),
        organization.id,
      );
    ...
    // Otherwise, use credit purchase flow
      ...
      result = await purchaseOrganizationCredits(
        priceId,
        data.credits,
        organization.id,
      );
