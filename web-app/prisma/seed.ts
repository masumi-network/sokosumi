...
const dummyAgents = [
    {
        ...
        exampleOutputs: [
            { name: "Market Report", mimeType: "application/pdf", url: "https://example.com/market-report.pdf" },
            { name: "Trend Analysis", mimeType: "application/json", url: "https://example.com/trend-analysis.json" }
        ]
    },
    {
...
async function main() {
    ...
    for (const agent of dummyAgents) {
        console.log(`Seeding agent ${agent.title} (${index + 1}/${dummyAgents.length})`);
        index++;
