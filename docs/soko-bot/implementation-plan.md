...
- Behaviour lab is admin-only (platform `admin` role), shows only runs of the selected version, and renders the version overview (model, skills, tools, prompt). `pnpm --filter core soko-bot:lab -- --user <id> --version <id>` does the same headlessly and writes a JSON report.
