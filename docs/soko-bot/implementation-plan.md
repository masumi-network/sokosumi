...
## Agent presets and step detail (2026-08-25)

- Presets live in `packages/soko-bot/src/presets.ts`: id, model (AI Gateway id), optional tool allowlist, optional extra system guidance, skills (documented). Add a new entry instead of editing one with lab history. `SokoBot.presetId` selects the bot's preset (`PUT /v1/soko-bots/me/preset`); each turn records `presetId`.
- Core signs the preset into the runtime request token (`model`, `presetId`, `presetInstructions`); Eve resolves the model per session (`agent.ts` dynamic model) and appends the guidance as turn-scoped system instructions (`agent/instructions/preset.ts`). `SOKO_BOT_AGENT_MODEL` is gone — the compiled default is only a fallback.
...
- Behaviour lab is admin-only (platform `admin` role) and runs against the bot's current preset; `pnpm --filter core soko-bot:lab -- --user <id> --preset <id>` does the same headlessly and writes a JSON report.
