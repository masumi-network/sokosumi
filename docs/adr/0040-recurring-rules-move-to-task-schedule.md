...
A repeating rule is a **Task Schedule**, its own model, not a Task. It holds the rule and the blueprint (name, description, project, visibility, any assignee kind) and is Active, Paused, or Ended. Each Occurrence creates a new Ready Task carrying `scheduleId`. A Task itself keeps only a single **Run at**; it never repeats. The API lives at `/v1/tasks/schedules`; the old per-Task schedule routes and `POST /v1/tasks/scheduled` answer `410 task_schedule_moved`.
