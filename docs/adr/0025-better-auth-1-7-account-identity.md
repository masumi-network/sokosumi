...
Better Auth **1.7.3** restored 1.6 account identity `(providerId, accountId)` and dropped the required `issuer` column. Microsoft `oid` rewrites from this cutover stay. The Prisma migration `20260907080000_better_auth_1_7_3_drop_account_issuer` removes `account.issuer` and `account_issuer_accountId_key`.
...
We never invent `oid`, never merge users by email, and never rewrite `accountId` while 1.6 is the running binary. Unverified password users cannot implicit-link Google/Microsoft (Better Auth 1.7 default; `requireLocalEmailVerified` is omitted). Creating a Google or Microsoft account marks `emailVerified`. A 1.7 magic-link verify on a still-unverified user deletes every account (password included) and standing sessions, then sets `emailVerified`. Google/Microsoft-linked users are already verified, so that wipe does not strip those links.
