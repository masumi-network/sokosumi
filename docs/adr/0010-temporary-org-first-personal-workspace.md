...
Creating or joining an organization still lands the user in that org. A temporary overlay can also create a personal workspace on that path (`beforeCreateOrganization`, `beforeAddMember`, `beforeAcceptInvitation`, join-link accept, admin add-member) without clearing `preferredOrganizationId`. The overlay is off unless Core `REQUIRE_PERSONAL_WORKSPACE=true` (default `false`, matching ADR 0005). Existing org-only users are backfilled with `pnpm data-migration:org-only-personal-workspaces`, which uses `packages/database/.env` like Prisma CLI (not Core env). If backfill finds `preferredOrganizationId` null, it sets it to an existing org membership so session create does not drop them into personal.
