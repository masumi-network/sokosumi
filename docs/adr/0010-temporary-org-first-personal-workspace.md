...
Creating or joining an organization still lands the user in that org. A temporary overlay also creates a personal workspace on that path (`beforeCreateOrganization`, `beforeAddMember`, `beforeAcceptInvitation`, join-link accept, admin add-member) without clearing `preferredOrganizationId`. Existing org-only users are backfilled the same way (`pnpm data-migration:org-only-personal-workspaces`). If backfill finds `preferredOrganizationId` null, it sets it to an existing org membership so session create does not drop them into personal.
