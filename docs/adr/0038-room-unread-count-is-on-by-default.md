...
**Stored as "hide", not as a new default on "show".** `showRoomUnreadCount` was `NOT NULL DEFAULT false`, so every existing reader held `false`, and a reader who never chose could not be told from one who switched the count off. Flipping that column's default would have meant rewriting every row and guessing at intent. The preference is a new column instead, `hideRoomUnreadCount`, `NOT NULL DEFAULT false`. Its `false`, which every reader starts with, means shown, so every reader lands on the new default with no backfill.
...
**Rejected:** flipping `showRoomUnreadCount`'s default and backfilling `true` (it could not tell "never chose" from "chose off", and after the write nobody can); a nullable `showRoomUnreadCount` with null meaning default (the column was already non-null and populated, so every row would still have needed rewriting to null, with the same guess); removing the switch (a reader for whom the numbers are too much keeps a way out); renaming the wire field (it would break the Apple client for a name only Core's storage cares about).
