...
- Status: Proposed
- Date: 2026-09-19
- Amended: 2026-09-20 (consumer-placement spike — see Amendment)

Core keeps its stack: **Hono + `@hono/zod-openapi`, TypeScript, Zod 4, Prisma on Postgres, Better Auth, Vitest, Biome, Turborepo, evlog, Vercel AI SDK**, deployed as a single Vercel function with Fluid compute. We are **not** moving Core to a long-lived container (Fly, Railway, ECS). Exactly **one** change is proposed: a real job queue for background work. Everything else considered was either rejected or found to be already true; see Considered options.
...
What Core lacks is a **queue**. Scheduled work is 17 Vercel crons calling `/sync/*` GET routes, six of them every minute, each contending on one `Lock` row through `syncLockService`. That is a polling scheduler assembled from cron and a lease lock, and **no `/sync` tick retries** — a throw is lost until the next tick fires. A container would have inherited the same gap.

## Amendment, 2026-09-20

The consumer-placement spike overturned one premise and sharpened two numbers. The decision to adopt a queue is unchanged; **which** queue is now genuinely open.
...
**A hybrid is therefore live**: `pg-boss` for the money paths, Vercel Queues push mode for the paths where at-least-once from an external system is fine — which would retire the `soko-bot` crons entirely.

**Not yet checked:** dead-letter support in Vercel Queues, and what `v2beta` implies for stability and migration guarantees.
...
- If `pg-boss` is chosen for a path, the existing crons can be its consumer: `fetch()` / `complete()` / `fail()` with `batchSize` drains inside the loop [`routes/sync/handler.ts`](../../apps/core/src/routes/sync/handler.ts) already implements. Do **not** reach for `supervise: false` to make it lighter — that disables the monitor that expires stuck jobs, which is one of the reasons to adopt pg-boss at all, and the docs call it an advanced case.
...
- If Vercel Queues is chosen for a path, that path's cron entry and lock key disappear entirely — no drain loop to write, and nothing to delete later.
- [`sync-lock.service.ts`](../../apps/core/src/services/sync-lock.service.ts) and the `Lock` Prisma model are deleted once **every** `/sync/*` route has moved, whichever queue each one lands on; that is a schema migration. The `crons` block in `apps/core/vercel.json` shrinks to whatever still drives a queue.
