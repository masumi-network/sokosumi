...
- The browser token capability set and the `ABLY_SUBSCRIBE_ONLY_KEY` dashboard key must also carry `presence` on the `notifications` namespace: that is how a tab or app says it is in front of the reader. A key without it has every client enter refused, the channel reports nobody present, and every notification email goes now (SOK-1090).
- The `ABLY_SUBSCRIBE_ONLY_KEY` dashboard key must also carry `channel-metadata`: Core reads the occupancy of a user's notifications channel with it, where a tab or app enters presence while it is in front, to decide whether a notification email goes now or after the category's delay (SOK-1090). Without it every email goes now and each occupancy read is reported to Sentry.
