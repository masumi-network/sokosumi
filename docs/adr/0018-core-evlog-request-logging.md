...
`evlog/better-auth` is not a Better Auth `plugins[]` entry and does not emit sign-in/sign-up events. Core uses the documented Hono pattern: `createAuthMiddleware(auth)` after `evlog()`, with `exclude: ["/auth/**", "/sync/**", "/debug/**"]` (Sokosumi auth is `/auth`, not `/api/auth`; crons and debug must not cookie-identify), `session: false`, and `fields: ["id", "email"]` (id + masked email — no avatar, session, or UA). Cookie-session routes therefore call `getSession` twice (evlog identify + Core `sessionMiddleware`); Better Auth cookie cache keeps that cheap. API keys, coworker keys, and the orchestrator token still use the actor/id enrichers.

`log.error` is only applied for 5xx / unexpected errors (same gate as `shouldReportHttpException`). Expected 4xx stay `level: info` with `status` on the event so Sentry Logs is not flooded with client errors. `/v1/openapi.json` is excluded from emit.
