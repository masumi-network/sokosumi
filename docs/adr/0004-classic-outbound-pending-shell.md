...
- **Chrome (timestamp slot):** pending keeps **wall-clock** for a short delay (~500ms); only if still pending after that delay show a quiet dual-arc **spinner** (bad-network path; static under reduced motion). Confirm before the delay → wall-clock only (no spinner, no check). Confirm after the delay → brief **check** that fades into wall-clock. Failed = alert immediately + Retry/Remove under the body. Toast only if a failed shell cannot be shown (e.g. surface unmounted). Not coworker Drive/thinking chrome.
