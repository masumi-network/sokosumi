...
- **Chrome (timestamp slot):** pending = quiet dual-arc ring (slow spin; static under reduced motion) where wall-clock time usually sits; confirm = brief check that fades into real wall-clock time; failed = alert in that slot + Retry/Remove under the body. Toast only if a failed shell cannot be shown (e.g. surface unmounted). Not coworker Drive/thinking chrome.
