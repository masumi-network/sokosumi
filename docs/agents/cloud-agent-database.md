...
| Trigger | Mechanism |
| --- | --- |
| PR merged | GitHub Action `cloud-agent-db-teardown.yml` on `pull_request_target` closed; extracts `bc-…` ids from PR body (Cursor agent links). Checks out the default branch, never the PR tree. |
