...
Parity rows run one per fresh session: the user types `/apple-parity-next` (first-party, user-invoked, [`.agents/skills/apple-parity-next/`](../../.agents/skills/apple-parity-next/)), which carries the next `apps/apple/PARITY.md` row from dispatch to a merged PR.
