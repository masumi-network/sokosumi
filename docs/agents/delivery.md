...
- **CI jobs restore caches; only `.github/workflows/js-cache.yml` saves them, from `main`.** A PR's cache is scoped to its merge ref, so no other PR can read it, and PR-saved copies once filled the repo's 10 GB. Setup restores `node_modules` (`js-node-modules-*`, keyed on the lockfile) and Build restores web's Turbopack cache (`js-next-*`). Keep new keys under `js-` so they never collide with Apple's `macOS-mint-*`. Build env placeholders live in `.github/ci-build.env`, shared by both workflows.
