...
- **Advisory status checks** in `.github/workflows/test.yml`: `Test CLI`, `Test Local env`, `Test CI config`, `Test Cloud agent db`. They are **not** in the ruleset; a red advisory job does not block merge. `Test CI config` runs on Markdown, `package.json`, and CI-file changes, so the doc-script check still executes on docs-only PRs.
