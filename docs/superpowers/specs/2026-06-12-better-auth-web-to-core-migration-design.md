...
| `stripe()` plugin | `stripeClient` instance, `STRIPE_WEBHOOK_SECRET` (single endpoint, see below), `createCustomerOnSignUp: false`, subscription plans from **core's existing** `subscription-catalog.service` (#3127) adapted to the plugin's `plans` shape, `getCheckoutSessionParams`, `authorizeReference` via `memberRepository`, `onSubscriptionCreated/Update` → ported `reconcileActiveStripeBackedSubscription`, `onEvent` → delegates every event to `stripeWebhookService.handleEvent`. **Single-webhook decision (user, 2026-06-12):** core's own `POST /webhooks/stripe` receiver (#3126) is deleted; the Better Auth plugin endpoint `<core-host>/auth/stripe/webhook` is core's only Stripe webhook, exactly like web's old setup. `stripeWebhookService` keeps invoice.paid / customer.created / customer.updated and gains customer.subscription.deleted. Retry semantics: handler throws → plugin responds non-2xx → Stripe retries (Stripe retries any non-2xx). |
...
2. Copy to core projects (⚠️ Google/Microsoft secrets are **required at boot** — core's whole API fails env validation without them, not just auth): `GOOGLE_CLIENT_ID/SECRET`, `MICROSOFT_CLIENT_ID/SECRET`, `BETTER_AUTH_RP_ID` (exact current web value), `BETTER_AUTH_COOKIE_DOMAIN=.sokosumi.com` (and preview equivalent), GTM webhook URLs — note the names changed: web `USER_CREATED_WEBHOOK`/`USER_UPDATED_WEBHOOK`/`ACCOUNT_CREATED_WEBHOOK` → core `WEBHOOK_USER_CREATED`/`WEBHOOK_USER_UPDATED`/`WEBHOOK_ACCOUNT_CREATED`, password length values if env-driven, `BETTER_AUTH_PROFILE_PICTURE_TIMEOUT`, `BETTER_AUTH_EMAIL_VERIFICATION_EXPIRES_IN`.
...
4. Stripe dashboard: create webhook endpoint → `<core-host>/auth/stripe/webhook` subscribed to **all** core-handled events: `checkout.session.completed`, `customer.subscription.created/updated/deleted`, `customer.created`, `customer.updated`, `invoice.paid`. **Replace** core's `STRIPE_WEBHOOK_SECRET` env with this endpoint's signing secret (the old value signed the now-deleted `POST /webhooks/stripe` receiver). Delete the old core `/webhooks/stripe` dashboard endpoint and keep web's endpoint until post-verify.
