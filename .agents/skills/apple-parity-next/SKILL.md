...
2. **Find open work.** A row branch is `claude/apple-parity-<id>-<slug>` with `<id>` starting with a digit. One open row PR: go to step 5 with it. More than one: stop and ask. None open, and the newest row PR is closed unmerged: stop and ask. Done when you know whether one is open.
