...
3. **Branch: is this a multi-session build?**
   ...
   Either way, **`/implement`** builds each issue by driving **`/tdd`** internally (one red-green slice at a time), then closes out by running **`/code-review`**, a two-axis review (Standards + Spec) of the diff, before committing. After the PR exists, **`/branch-bugbot-gate`** is mandatory on Sokosumi product work: local verify exit 0, CI green, Bugbot zero High; Medium findings go to Linear (when `SOK-XXX` is known) or a PR comment for the human merge pass. Reach for **`/tdd`** on its own when you just want to build a concrete behaviour test-first without a full spec, and **`/code-review`** on its own whenever you want to review a branch or PR against a fixed point.
