...
- **Auth for a test account:** on a provisioned agent Neon branch, use fixtures `alice@sokosumi.test` / `bob@sokosumi.test` with password `Password123!` (upserted after migrate; agent branches only). Otherwise email/password signup works with no email verification and auto sign-in (`/signup`). Google/Microsoft OAuth and magic-link email do **not** work (placeholder credentials).
