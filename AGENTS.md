...
**Chat (web)**: Production **`/chat`** streams via **`/api/chat`** → Core **`POST /v1/conversations/chat`** (legacy). Experimental **`/new-chat`** streams via **`/api/new-chat`** → Core **`POST /v1/conversations/new-chat`** (Vercel AI SDK + `@sokosumi/ai-provider`). **`POST /api/chat/ai-sdk`** aliases **`/api/new-chat`**.
