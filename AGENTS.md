...
- **Computer-use default login:** use `VERIFY_SOKOSUMI_EMAIL` / `VERIFY_SOKOSUMI_PASSWORD` (same vars as `verify-sokosumi sign-in`). Runtime secrets `login_email` / `login_pwd` are copied into those vars when unset. Type into `[data-testid="auth-field-email"]` / `[data-testid="auth-field-currentPassword"]`, then Enter. Never print the password. Fall back to Alice fixtures when VERIFY_* is empty.
