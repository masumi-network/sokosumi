...
- **Computer-use login (mandatory order — do not invent users):**
  ...
  4. **Never invent** random `/signup` users (`you-*@sokosumi.test`, timestamp emails, etc.) for general UI proof. If `verify_credentials_*=unset` and Alice fixtures fail (`fixture_auth=fail`), **stop and report missing secrets** — do not improvise an account. Signup is only for testing signup itself (see `features/sign-up.md`).
