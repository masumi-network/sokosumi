...
- **Seed local fixture data:** after migrate (or `pnpm prisma:migrate:reset`), run `pnpm prisma:seed` in a login shell with local `DATABASE_URL`. Fills users, orgs, catalog, coworkers, tasks — fixes empty agents catalog / missing `credit_cost`. See `packages/database/prisma/SEED.md` for fixture emails and password. Prisma 7 does not auto-seed on reset.
...
- **Agents catalog is empty and 500s by default:** `GET /v1/agents` and `/v1/categories` throw `Failed to get credit information for agents` until the `credit_cost` table has rows. Run `pnpm prisma:seed` (local dev) or seed via admin `POST /v1/credit-costs` / `/admin` UI. This breaks the Agents marketplace and Chat/Tasks landing pages until seeded — missing data, not a broken build.
