...
Issues live in Linear (team "Sokosumi", key `SOK`). For all Linear work, run the `linear` CLI (`schpet/linear-cli`) in the terminal. Do not call Linear MCP tools (`linear__*`, `search_tool` for the `linear` server, `use_tool` with `linear__…`). If a Linear MCP tool is offered, ignore it and use `linear`. See [`docs/agents/issue-tracker.md`](./docs/agents/issue-tracker.md).
