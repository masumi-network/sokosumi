...
1. CLI-only read-only discovery. In `apps/cli` only, call the existing `GET /v1/vendors/me` and `GET /v1/users/me/organizations` routes to expose `vendors me` and `workspaces list`. Vendor discovery keeps only memberships with the `admin` role, so non-admin Vendors never become setup candidates. The `workspaces list` command returns organization-workspace candidates from `/v1/users/me/organizations`; each item exposes `organizationId`, never a Workspace row ID, `workspaceId`, or organization metadata. Non-array collection data or a missing Vendor or organization identity makes discovery fail instead of printing an empty or `unknown` selectable candidate. Each command emits stable text by default or exactly one JSON document on stdout with `--json`. This PR includes no Core edits, Vendor creation, Coworker registration, or TUI work.
   ...
   - Allowed: authenticated exact commands, administered Vendor candidates, organization-workspace candidates, text output, one-document JSON. Denied: unauthenticated Core calls, bare/wrong subcommands, non-admin Vendor candidates, malformed lists, missing IDs, organization metadata output, Vendor creation, registration, and TUI changes.
