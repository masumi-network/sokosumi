...
1c. [DONE / PR developer Vendor create] Developer self-service Vendor create: Core `POST /v1/vendors` (caller becomes admin); CLI `vendors create` and `coworkers register --create-vendor --confirm-create-vendor --vendor-name --vendor-slug`. Platform-admin create remains for ops.
