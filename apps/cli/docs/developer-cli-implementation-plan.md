...
1b. [DONE 2026-09-21 / SOK-966 PR2] Controlled registration selection. `coworkers register` and the TUI Register screen require at least one organization workspace and an administered (`admin`) Vendor. Foreign/non-admin Vendor ids fail before Core create. Gate copy directs blocked developers to `sokosumi vendors create`; Coworker create remains platform-admin only. Discovery commands stay read-only.
1c. [DONE / PR developer Vendor create] Developer self-service Vendor create: Core `POST /v1/vendors` (caller becomes admin); CLI standalone `vendors create --name --slug`. `coworkers register` uses only `--vendor-id` (administered) and never creates a Vendor; Coworker create stays platform-admin only. Platform-admin Vendor create remains for ops.
