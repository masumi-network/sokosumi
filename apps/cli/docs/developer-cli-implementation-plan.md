...
1b. [DONE 2026-09-21 / SOK-966 PR2] Controlled registration selection. `coworkers register` and the TUI Register screen require at least one organization workspace and an administered (`admin`) Vendor. Foreign/non-admin Vendor ids fail before Core create. Gate copy directs blocked developers to ask an existing Vendor admin to add them as admin (platform admin create is the fallback). Discovery commands stay read-only.
