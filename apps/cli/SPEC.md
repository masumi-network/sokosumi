...
apps/cli slice 1: Developer CLI login/signup via existing Better Auth OAuth. Thin Ink status screen. After sign-in, pick a Coworker runtime (pi-sokosumi / Eve / Hermes / OpenClaw). Core first-party client `sokosumi_cli`. ⊥ marketplace TUI. ⊥ workspace connect / chat / Tasks (later slice).
...
- cmd: `sokosumi` (no args) → Ink: signed-out Sign in | signed-in status (not email) + Register a Coworker (pi-sokosumi / Eve / Hermes / OpenClaw) + Sign out
...
V9: persist session only in OS keychain. Fail closed if keychain cannot store. ⊥ `~/.sokosumi` credentials file.
V10: signed-in identity copy is whether signed in. ⊥ email, ⊥ name on the status screen.
V11: Register menu presets ∈ {pi-sokosumi, Eve, Hermes, OpenClaw}. Those are Coworker runtimes. ⊥ Hire Agent. Connecting to one workspace (chat + Tasks) is a later slice.
...
T8|.|coworker connect loop (later slice)|V5,V6,V11
T9|x|signed-in Register a Coworker menu (preset pick only)|V10,V11
