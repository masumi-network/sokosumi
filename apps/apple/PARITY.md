...
- Active slice: pinned messages (21), branch `codex/apple-pinned-messages`, rebased onto main after prerequisite #4546 merged. Channel-only, top-level pins now have a native inspector, paginated previews, pin/unpin actions, realtime invalidation, and message jumping with a separate historical window and return-to-latest. No Apple PR yet; final verification remains in progress.
...
- Current pin verification: 349 Chat tests, 68 Workspace tests, pinned SwiftFormat and strict SwiftLint pass. The Workspace tests cover an in-flight pin operation while sending from historical messages: room identity protects completion independently of timeline reloads. Initial Xcode app build/tests and all five package suites passed (349 Chat, 68 Workspace, 34 Auth, 27 Realtime, 1 CoreAPI). Existing Organization.logo generator, AppIntents metadata and cmark module warnings remain. iOS 17 compilation passed using SwiftPM with the installed simulator SDK (`/tmp/apple-pins-ios17-swift.log`); Xcode could not select a generic simulator destination. Live inspection confirmed native/web empty-panel layout, channel-only toolbar availability, the native Pin context-menu action, and quote navigation. Populated cards were rendered at 340-point inspector width in both light and dark appearance, including short, long and unavailable previews. The new app-host regression test caught and verified the fix for short previews expanding to the height cap. Screenshots are under `docs/screenshots/pinned-messages-{light,dark}.png`. A full app run failed the existing scrolling-distance assertion while iOS compilation ran concurrently; the complete suite passed on rerun without concurrent compilation (`/tmp/apple-pins-app-tests-final3.log`). This does not establish a scrolling code fix. Live pin/unpin mutations remain unverified; no live messages or pins were changed.
...
| 21 | Pinned messages: pin/unpin, list, loading/empty/error, jump to message and realtime updates. | 07 | Partial — implemented; preparing draft PR | [pinned-messages-panel.tsx](<../web/src/app/(app)/chat/components/pinned-messages-panel.tsx>), [rooms-client.tsx](<../web/src/app/(app)/chat/components/rooms-client.tsx>), [actions.ts](<../web/src/app/(app)/chat/actions.ts>) |
