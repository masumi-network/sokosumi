...
- Current slice: chat start surface (28), implementation in draft [#4669](https://github.com/masumi-network/sokosumi/pull/4669), branch `codex/apple-chat-start`. Scope confirmed: match current web. Welcome selects a coworker and opens or reuses the conversation; compose the first message in the room. Native coworker List, ranked default selection, existing Direct handoff and explicit Welcome navigation are implemented; local verification passed; CI and human merge are pending.
...
| Device-local Auto/12-hour/24-hour time preference merged in [#4618](https://github.com/masumi-network/sokosumi/pull/4618), [time-format.ts](<../web/src/i18n/time-format.ts>) | Add explicit preference to Settings (39). Web stores browser cookies; this is not an account-synced Core contract. Native system formatting remains the default until that slice. |
| Core outage distinguished from logout ([#4365](https://github.com/masumi-network/sokosumi/pull/4365)) | Auth uses a different native OAuth boundary. Verify transient Core/refresh failures preserve recoverable session state before closing auth maintenance; this audit does not establish a native defect. |
...
| 28 | Chat start surface: select an available chat coworker, defaulting to the highest priority (slug tie-break), and open or reuse its Direct with duplicate-open protection. Compose the first message after entering the room; conversation navigation stays in the sidebar. Include availability notices. Native empty/start UI replaces the web layout; the landing's task statistics strip and search-agents CTA are excluded (see Out of scope). | 03, 27 | Draft [#4669](https://github.com/masumi-network/sokosumi/pull/4669) | [chat-landing.tsx](<../web/src/app/(app)/chat/components/landing/chat-landing.tsx>), [landing-coworker-picker.client.tsx](<../web/src/app/(app)/chat/components/landing/landing-coworker-picker.client.tsx>), [chat-landing-notice.tsx](<../web/src/app/(app)/chat/components/chat-landing-notice.tsx>), [use-open-coworker-room.tsx](<../web/src/app/(app)/chat/components/landing/use-open-coworker-room.tsx>), [landing-content.ts](<../web/src/app/(app)/chat/components/landing/landing-content.ts>) |
...
| 39 | Minimal Settings: account identity/sign out, chat unread-count display preference with persisted server state and rollback; device-local Auto/12-hour/24-hour time format matching the web preference. | 01, 05 | Partial | [chat-display-preferences.tsx](<../web/src/app/(app)/account/components/chat-display-preferences.tsx>), [account-menu-config.tsx](<../web/src/app/(app)/components/sidebar/components/account-menu-config.tsx>) |
...
## Slice 28 investigation — current web 2026-09-16

At `447104b82`, Welcome is composed in `apps/web/src/app/(app)/(welcome)/page.tsx`. It loads available chat coworkers via `coworkerService.listCoworkers("chat")`; Core owns workspace availability, and web checks chat capability and a runnable endpoint. `landing-content.ts` picks the highest-priority coworker with slug as tie-break. `StartChatButton` calls the existing ensure-Direct action, updates the sidebar before navigating, and guards duplicate opens. Chat room lists remain in the sidebar instead of being repeated below Welcome.

The old `stashPendingRoomMessage` has no production callers; only its utility tests call it. Welcome currently has no composer, though room code still reads the legacy pending storage. The user confirmed matching current web on 2026-09-16. Row 28 now requires coworker selection and room opening, with first-message composition in the existing room composer. Do not port the obsolete landing draft mechanism. This resolves the scope question; implementation evidence follows.

Reuse candidates: the existing Direct coworker endpoint/filtering and `WorkspaceState.openDirect` can own room reuse, sidebar insertion and stale-context guards. The existing sidebar already owns conversation navigation. The Apple selection coordinator currently auto-selects a room after refresh, so a deliberate Welcome route must remain distinct from transient nil list selection if added. No duplicate networking or sending path is justified.


## Slice 28 — native chat start

A sidebar “Start a chat” action opens a native welcome surface, also used when no room is selected. It loads only available chat coworkers using the existing Core call and filtering, shows avatars and trimmed captions in a native List, and selects the highest priority by default (slug tie-break). Start chat uses the existing guarded Direct creation/reuse path, inserts the returned room in the sidebar, and opens the existing room composer. Loading, empty, retry and creation-failure states are included. Existing unavailable deep-link alerts remain in ChatRootView. Conversation navigation remains in the sidebar; no landing composer, task statistics or agent-search CTA is added.

Explicit Welcome navigation survives background room refresh. A navigation generation prevents an outstanding Direct request from pulling the reader back after leaving or reopening Welcome. Empty workspaces remain on Welcome when new rooms arrive. Existing saved-room restoration remains the native launch behavior.

Verification: macOS Debug build and full app tests passed; all five package suites passed (Chat 416, Workspace 84, Auth 35, Realtime 27, CoreAPI 1). iOS 17 Workspace package build passed; strict uncached SwiftLint/SwiftFormat passed. The final navigation-generation/empty-workspace refinement passed the full Workspace suite and a fresh macOS build. Light/dark native-window fixtures were inspected: [light](docs/screenshots/chat-start-light.png), [dark](docs/screenshots/chat-start-dark.png). The test host exited. These fixtures cover rendering and loading, not authenticated creation, physical keyboard selection or live hover; those interactions remain unverified.

Slice 28 delivery: draft [#4669](https://github.com/masumi-network/sokosumi/pull/4669). Wait for CI and human merge before beginning slice 29.
