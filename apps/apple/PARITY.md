...
- Room search is implemented in draft [#4612](https://github.com/masumi-network/sokosumi/pull/4612), branch `codex/apple-room-search`, rebased onto current main. All package suites, the full app suite, macOS/iOS 17 builds, strict lint/format and normal commit hooks pass locally. Signed-app verification covers Find focus, typing, empty results, Escape clear/close, keyboard selection, room highlights, and an older reply opening in its parent thread with highlight. Pagination verification exposed an existing loading-row removal jump; controls now live in the persistent reply-count header, and all pagination/search-jump cases pass ten repetitions with the original two-pixel tolerance. All agent-launched apps are closed. Next: CI and review on #4612; do not start another slice before human merge.
...
| 23 | Room search: expanding header field and Find shortcut; 250 ms debounce, up to 50 results with sender/time, two-line previews and reply badge; keyboard selection, empty/error states, room/thread context loading and landed-message highlight. No load-more or query-term highlighting, matching current web. | 04, 10 | In review — [#4612](https://github.com/masumi-network/sokosumi/pull/4612) | [room-search-panel.tsx](<../web/src/app/(app)/chat/components/room-search-panel.tsx>), [use-room-message-jumps.ts](<../web/src/app/(app)/chat/hooks/use-room-message-jumps.ts>), [room-search-jump.ts](<../web/src/app/(app)/chat/utils/room-search-jump.ts>) |
