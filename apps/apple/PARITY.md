...
- Review follow-up: classic-send retry now asserts `mentionedUserIds` on both attempts. The Workspace test transport is explicitly main-actor isolated, removing unchecked shared pause/resume state that intermittently hung the room-switch test. All 55 Workspace tests passed in 20 consecutive runs; pinned SwiftFormat and SwiftLint passed. Active follow-up: [PR #4406](https://github.com/masumi-network/sokosumi/pull/4406) on `codex/human-mention-identity`, based on main `80a652c4e`, for ID-only human mention tokens across web and Apple, preserving existing `@userId:slug` mentions. Human picker rows show the display name with email underneath instead of a name-derived slug on both clients. The user approved the required web and shared-contract changes on 2026-09-11. Verify parser boundaries and identity resolution before implementing; this approval is limited to human mentions, not dependencies or unrelated web changes.

- Rich-composer [PR #4388](https://github.com/masumi-network/sokosumi/pull/4388) merged as `ef6f2e886` on 2026-09-10. All Apple CI checks passed on final head `af4cbdb32`. Slice 13 (mentions), [PR #4392](https://github.com/masumi-network/sokosumi/pull/4392), merged as `80a652c4e` on 2026-09-11. The approved human-mention identity follow-up is next.
