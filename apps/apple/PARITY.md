...
- Supported-language alias audit: added web aliases jsp, ipython, obj-c++ and objective-c++; existing Java/Python/Objective-C parsers are reused. All 11 highlighter tests and changed-file lint pass. GraphQL/LESS dependency approval remains pending.
- Registry audit against installed lowlight/common: remaining unsupported entries are arduino, graphql, less, php-template, python-repl, shell, vbnet and wasm (plaintext needs no parser). Existing native cases also include HTML/TOML/TSX. Detection remains incomplete. Do not substitute adjacent grammars without validating behavior or claim registry parity.
