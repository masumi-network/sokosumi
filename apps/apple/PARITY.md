...
- Active follow-up: thread-preview parity (24a), [draft #4629](https://github.com/masumi-network/sokosumi/pull/4629), branch `codex/apple-thread-preview-parity`, based on `origin/main` at `05b622d79`. Thread overview (24) merged in [#4626](https://github.com/masumi-network/sokosumi/pull/4626). Its review exposed a real difference from web’s shared thread-row/banner formatter. Implementation and local verification are complete, including macOS build, app/package tests, iOS 17 compilation and the web comparison corpus. Next: CI, review and human merge before slice 25. No new API or dependency is required.
...
- Current implementation: thread-preview parity (24a); room-scoped thread overview (24) is merged. Quick reactions (20b), room search (23), and shared read-budget cooldown (07b) are merged.
...
| 24a | Match web thread-row preview cleanup, mention names/fallbacks, address suppression and grapheme-safe 128-codepoint cap. Replace the earlier Markdown-flattening path; keep processing off the main thread. | 24 | In review — [#4629](https://github.com/masumi-network/sokosumi/pull/4629) | [unread-threads-preview.ts](<../web/src/app/(app)/chat/utils/unread-threads-preview.ts>), [chat-message-preview.ts](../../packages/utils/src/chat-message-preview.ts) |
...
Local implementation reuses the room tools inspector and thread coordinator. `ChatService+ThreadOverview` and `RoomThreadOverview` live in SokosumiChat; `WorkspaceState+ThreadOverview` coordinates authenticated requests. Thread list reads join the existing read cooldown. Implementation and local verification are complete; [draft #4626](https://github.com/masumi-network/sokosumi/pull/4626) has passing CI on `322e167d3d10aebc31a7185b797f3b5f687ca98f` and awaits review and human merge.
...
Preview parity remains incomplete for the documented sanitizer edge cases: web’s `formatUnreadThreadsPreview` delegates to `buildChatMessagePreview`, so that helper is not banner-only. Reusing native Markdown rendering does not prove identical preview behavior. Keep this gap open when evaluating slice 24 completion.
