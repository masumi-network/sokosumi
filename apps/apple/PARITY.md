...
- Current slice: Room details/roster (31), branch `codex/apple-room-details`, based on latest origin/main. Implementation and macOS verification complete locally; iOS build and PR publication pending.
...
### Slice 31 — room details and roster (source audit)
...
Verification: macOS build and full app tests pass, including inspector regression coverage and native layout captures at 280-point width in light/dark. All five package suites pass (Chat 431, Workspace 84, Auth 35, Realtime 27, CoreAPI 1). Strict SwiftLint and SwiftFormat pass. Visually inspected [light](docs/screenshots/room-details-light.png) and [dark](docs/screenshots/room-details-dark.png): long topic wraps and member rows fit without clipping. The task-owned app test host exited. iOS 17 package build remains running. Authenticated Message actions, physical keyboard/VoiceOver, clipboard feedback, and large Dynamic Type remain unverified; the render fixture uses an unauthenticated coordinator and does not exercise Message buttons. Existing coordinator tests cover direct creation/navigation guards. No production messages or memberships were changed.
