...
- Active slice: thread overview (24), branch `codex/apple-thread-overview`, based on `origin/main` at `4436a8804`. The user confirmed “match web”: room-scoped thread overview only. Workspace-wide unread threads were a stale audit requirement and are removed. In progress: generated existing list/count/mark-all bindings, added portable service/model and initial pagination/retry tests. Inspector and coordinator wiring is implemented locally; CoreAPI, Chat, Workspace and the macOS app build pass, along with nine targeted tests covering pagination, retry, read failures, mention preservation, display preferences and late responses after reset. The server count preference is now read through the existing preferences endpoint. Remaining: final source review and draft PR. Native fixture interaction verified opening the inspector, opening a thread, and restoring the inspector with Back. App build log: `/tmp/thread-overview-app-build.log`. Draft PR preparation is in progress.
...
| 24 | Thread overview for the selected room: unread-first thread list with older-page loading, header thread-attention indicator honoring the count preference, two-line mention-aware previews, starter/time/reply counts, open thread and back navigation, mark opened thread looked and Mark all read. Opening the overview alone does not mark threads read. | 07, 08 | In progress — room scope approved | [thread-list-panel.tsx](<../web/src/app/(app)/chat/components/thread-list-panel.tsx>), [unread-threads-panel.tsx](<../web/src/app/(app)/chat/components/unread-threads-panel.tsx>), [thread-overview-unread.ts](<../web/src/app/(app)/chat/utils/thread-overview-unread.ts>), [actions.ts](<../web/src/app/(app)/chat/actions.ts>) |
...
## Slice 24 — room thread overview (in progress)
