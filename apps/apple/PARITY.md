...
- Active slice: a simple native transcript with correct concurrency, branch `codex/apple-chat-scroll-performance`, based on merged #4571. Draft [#4573](https://github.com/masumi-network/sokosumi/pull/4573) uses standard SwiftUI `ScrollView`/`LazyVStack`, stable message IDs and native scroll-position APIs. Keep background Markdown preparation, cancellation/stale-result guards, and the small bottom-follow handler needed for content growth. The September 14 user instruction defers further performance tuning; a new live profile is no longer a gate for this slice. Do not add custom virtualization, row-height caches, scroll heuristics, prefetch schedulers or dependencies. The geometry-based reader-motion fallback was removed after a larger content-growth regression left a 310-point bottom gap. Small/large growth and room/thread scrolling are covered by the existing fixture. This slice still takes priority over room search (23); wait for human merge before starting that feature. See [audit](docs/scrolling-performance-audit.md).
