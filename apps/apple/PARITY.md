...
- Active slice: quick reactions (20b), branch `codex/apple-quick-reactions`, based on merged main `2a7a20bf4`. Reuse the existing history and reaction action for three toolbar shortcuts; preserve order during interaction and fit narrow thread panes. Verification passed locally. Next: open one PR, then wait for human merge.
...
| 20b | One-tap reactions before React in the message toolbar; reuse frequent history with default fallback, stable choices while interacting, existing availability/own-reaction behavior, keyboard labels and hover feedback. | 20 | In progress — toolbar quick reactions | [quick-reactions.ts](<../web/src/app/(app)/chat/utils/quick-reactions.ts>), [room-message-row.tsx](<../web/src/app/(app)/chat/components/room-message-row.tsx>) |
