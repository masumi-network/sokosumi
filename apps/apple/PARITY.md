...
- Search slice implemented on `codex/apple-room-search`, based on merged [#4608](https://github.com/masumi-network/sokosumi/pull/4608). The existing room endpoint exposes `q`; the existing thread endpoint exposes `around`, so no contract or generated-file change is needed. Portable search state, native Find/results UI and room/reply navigation are implemented. All 364 Chat tests, 72 Workspace tests and the app suite pass locally, including light/dark result fixtures and scrolling to an older reply. The iOS 17 package build passes. Signed-app checks verified Find focus, immediate typing, empty results, Escape clear/close, and keyboard selection with a highlighted room-message landing. Focus must be owned inside the toolbar field view; owning it in the surrounding modifier did not focus the native field. A repeated pagination check exposed an existing prepend-layout defect, also reproduced with the pre-search view and model. Removing the standalone “Load older replies” row during insertion moved visible messages. The control now lives in the persistent reply-count header. All pagination/search-jump cases pass across ten repetitions with the original two-pixel tolerance; strict SwiftLint and SwiftFormat pass. The full app suite passes after the fix. Live verification opened an older search reply in its correct parent thread and highlighted the target. Missing-client errors retain the submitted query instead of displaying a pending spinner; all 73 Workspace tests pass, including that regression. The slice is ready for a draft PR. All agent-launched apps were closed.

- Audit refreshed on 2026-09-15 against `origin/main` at `808245bf311218a20972a0ccedace602ce5b13c5`. This is a source/history audit, not a fresh visual acceptance or test run. Earlier implementation evidence remains in the linked PRs and Git history.
...
- Next implementation: room search (23), matching current web as explicitly selected by the user. The documentation refresh merged as #4608. Prioritize the shared read-budget follow-up (07b) before adding further background readers; quick reactions (20b) can follow search independently.
...
| Shared read-budget cooldown: [throttle](<../web/src/lib/chat/chat-read-throttle.ts>), [scheduler](<../web/src/components/chat/use-chat-refresh-scheduler.ts>) ([#4552](https://github.com/masumi-network/sokosumi/pull/4552)) | New 07b. Apple room/thread/message/pin services recognize 429 but discard retry timing into a generic error; no shared cooldown was found. Preserve realtime delivery while delaying HTTP recovery reads. |
| Frequent/default one-tap toolbar reactions: [resolver](<../web/src/app/(app)/chat/utils/quick-reactions.ts>), [message row](<../web/src/app/(app)/chat/components/room-message-row.tsx>) ([#4593](https://github.com/masumi-network/sokosumi/pull/4593)) | New 20b. Apple has frequent emoji in the picker; that does not provide toolbar shortcuts. Reuse the existing reaction action and history. |
...
| 07b | Honor shared chat read-budget backoff across HTTP recovery readers: retain Retry-After/body delay, coordinate cooldown, bounded fallback and delayed resume; keep realtime events live and isolate account lifetime. | 07, 08 | Todo — new web delta | [chat-read-throttle.ts](<../web/src/lib/chat/chat-read-throttle.ts>), [use-chat-refresh-scheduler.ts](<../web/src/components/chat/use-chat-refresh-scheduler.ts>) |
...
| 20b | One-tap reactions before React in the message toolbar; reuse frequent history with default fallback, stable choices while interacting, existing availability/own-reaction behavior, keyboard labels and hover feedback. | 20 | Todo — new web delta | [quick-reactions.ts](<../web/src/app/(app)/chat/utils/quick-reactions.ts>), [room-message-row.tsx](<../web/src/app/(app)/chat/components/room-message-row.tsx>) |
...
| 23 | Room search: expanding header field and Find shortcut; 250 ms debounce, up to 50 results with sender/time, two-line previews and reply badge; keyboard selection, empty/error states, room/thread context loading and landed-message highlight. No load-more or query-term highlighting, matching current web. | 04, 10 | In review — implementation and local verification complete; draft PR being prepared | [room-search-panel.tsx](<../web/src/app/(app)/chat/components/room-search-panel.tsx>), [use-room-message-jumps.ts](<../web/src/app/(app)/chat/hooks/use-room-message-jumps.ts>), [room-search-jump.ts](<../web/src/app/(app)/chat/utils/room-search-jump.ts>) |
| 24 | Thread overview: room thread list, workspace unread threads, header thread-attention indicator, previews/counts, open target, mark one/all read. | 07, 08 | Todo | [thread-list-panel.tsx](<../web/src/app/(app)/chat/components/thread-list-panel.tsx>), [unread-threads-panel.tsx](<../web/src/app/(app)/chat/components/unread-threads-panel.tsx>), [thread-overview-unread.ts](<../web/src/app/(app)/chat/utils/thread-overview-unread.ts>), [actions.ts](<../web/src/app/(app)/chat/actions.ts>) |
...
Local checks: full SokosumiChat and SokosumiWorkspace suites; remaining CoreAPI/Auth/Realtime package suites; Xcode app suite, including search panel renders at 280-point inspector width in both appearances and a native scroll test for selecting an older reply. Keyboard, real search/jump acceptance, final signed build and iOS 17 compilation are still pending at this checkpoint. UI-free packages retain iOS 17 availability; no dependency is added.
