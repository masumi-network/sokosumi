...
- Current slice: conversation actions (26), draft [#4640](https://github.com/masumi-network/sokosumi/pull/4640); in-app links (25) merged in [#4630](https://github.com/masumi-network/sokosumi/pull/4630); thread-preview parity (24a) and room-scoped thread overview (24) are merged. Quick reactions (20b), room search (23), and shared read-budget cooldown (07b) are merged.
...
| 26 | Conversation actions: pin/unpin a conversation in the sidebar (web label Pin, Core star routes; message pins are row 21), mute/unmute, mark unread with optimistic rollback and sidebar ordering/attention. | 05, 07 | Draft [#4640](https://github.com/masumi-network/sokosumi/pull/4640) | [chat-room-sidebar-row.tsx](<../web/src/components/chat/chat-room-sidebar-row.tsx>), [organization-chat-list.actions.ts](<../web/src/components/chat/organization-chat-list.actions.ts>) |
...
The Core OpenAPI snapshot was regenerated with existing `/chats/rooms/{id}/star` and `/chats/rooms/{id}/mute` operations. No shared API change or new dependency is needed. Implementation adds the native context menu in web order, a trailing pin/mute indicator, dimmed muted titles, and a small pending-action indicator. The coordinator has one sidebar action path. Pending requests are invalidated on workspace/membership changes; only the changed timestamp is committed or rolled back. The sidebar now applies the existing unread overlay before partitioning, so Mark unread updates its visible row immediately.

Verification: 401 Chat tests, 82 Workspace tests, 35 Auth tests, 27 Realtime tests and one CoreAPI test pass. New tests cover all four star/mute routes and organization scope, availability, pending-action exclusion, refresh during/after an action, field-specific rollback for 401/403/404/422/500, workspace resets, removed rooms, and shared unread rollback. Strict SwiftLint/SwiftFormat, macOS build/app tests and an iOS 17 Workspace package build pass. Native 260-point sidebar fixtures were inspected in light/dark, including long-name truncation and indicator alignment: [light](docs/screenshots/sidebar-actions-light.png), [dark](docs/screenshots/sidebar-actions-dark.png). The temporary fixture was removed and its test host exited. Live authenticated context-menu interaction remains a manual acceptance check; screenshots prove row appearance, while HTTP/state tests prove action behavior. Draft PR: [#4640](https://github.com/masumi-network/sokosumi/pull/4640). CI and review are pending; wait for human merge before the next slice.
