...
- Active slice: scrolling architecture and performance audit, branch `codex/apple-chat-scroll-performance`, based on merged #4571. Draft [#4573](https://github.com/masumi-network/sokosumi/pull/4573) contains the audit, repeatable media fixture and boundary-coalescing fix; Release benchmarking passed; the September 14 live capture confirms scrolling still stutters. CI passes on `d0e112bb8`; user reports the renderer-isolated build feels better but is still not perfectly fluid. Renderer isolation reduces unrelated-notification body evaluations from ten to one in the diagnostic probe. The initial raw-text fallback now uses a small native loading indicator; previous formatted content remains visible during updates. Full app tests and changed-file lint/format pass locally. Prepared room/thread snapshots now parse off-main before publishing rows; unchanged documents are reused and cancelled results cannot publish. Final serial layout p95 is 0.084 ms for room text and 0.100 ms for thread text; content-growth checks pass. All 357 Chat tests and full app tests pass. User feedback on the prepared-snapshot build: improved, but still stutters. Further live profiling is required; performance acceptance remains open. This user-requested slice takes priority over room search (23). See [audit](docs/scrolling-performance-audit.md).
