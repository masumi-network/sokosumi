...
- Rich-composer [PR #4388](https://github.com/masumi-network/sokosumi/pull/4388) merged as `ef6f2e886` on 2026-09-10. All Apple CI checks passed on final head `af4cbdb32`. Slice 13 (mentions) is implemented on `codex/apple-chat-mentions`, rebased onto main `5f2476074`, and being prepared for review.
...
- Branch: `codex/apple-chat-mentions`, rebased onto main `5f2476074`. Implementation is ready for a draft PR after the final package checks.
...
- Native adaptations: completion uses the system menu; participant details use click/keyboard popovers. Code and existing links retain their literal content rather than becoming references. Presence remains the roster snapshot; live presence belongs to its later slice.
- Verification: 403 package tests passed before final reference guard and navigation-case additions; four reference tests and all three Direct lifecycle cases subsequently pass. Full native tests pass, including the @ shortcut and chip editing. Signed app build passed; shared Chat and dependencies compiled for arm64 iOS 17. Final formatting and strict lint checks pass.
