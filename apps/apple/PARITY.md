...
- Maintenance [PR #4385](https://github.com/masumi-network/sokosumi/pull/4385) merged as `e71173878`; all Apple CI jobs passed. Sources are grouped by feature and the coordinator lives in `SokosumiWorkspace`. Current branch: `codex/apple-rendering-verification`, based on that merge; no open PR.
...
| 12a | Emoji entry: native searchable character picker, exact shortcode conversion, boundary-triggered emoticons and trailing conversion before sending. Independent of rich formatting; partial-shortcode suggestions remain 12b. | 06 | Partial — implementation in progress | [composer-wysiwyg-editor.tsx](<../web/src/components/chat/composer-wysiwyg-editor.tsx>), [composer-emoticons.ts](<../web/src/lib/utils/composer-emoticons.ts>), [emoji-shortcodes.ts](<../web/src/lib/utils/emoji-shortcodes.ts>), [emoji-picker.tsx](<../web/src/components/chat/emoji-picker.tsx>) |
| 12b | Rich composing: bold/italic/underline/strike, inline/block code, quote, ordered/unordered lists, insert/edit link, partial-shortcode suggestions, formatting shortcuts and toolbar visibility. Split from 12a because formatting and emoji entry can ship independently. | 06, 10 | Todo | [composer-wysiwyg-editor.tsx](<../web/src/components/chat/composer-wysiwyg-editor.tsx>), [composer-format-toolbar.tsx](<../web/src/components/chat/composer-format-toolbar.tsx>), [composer-add-link-dialog.tsx](<../web/src/components/chat/composer-add-link-dialog.tsx>) |
...
- Native IME test passes. Undo/redo regression test exposed duplicate whitespace on redo; the fix separates native undo coalescing around the replacement and suppresses conversion during replay. The final Xcode app test run passed (`/tmp/apple-emoji-xcode.log`), including undo/redo and IME tests. Native picker button focuses the composer, but the separate system panel is not observable through computer control and still needs a manual check. Final review and PR remain pending. Changes remain uncommitted; do not treat 12a as complete.
