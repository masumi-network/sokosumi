...
- Native highlighting checkpoint: 31 grammar choices and common fence aliases integrated, including Kotlin declarations, types, control flow, annotations and braced interpolation; 208 Chat tests, Xcode app build/tests (including bundled query lookup), iOS 17 compilation and lint/format passed. Full registry, aliases/detection and remaining rich-text behavior are still incomplete.
...
- Parser/emoji prerequisite [PR #4374](https://github.com/masumi-network/sokosumi/pull/4374) merged on 2026-09-10. SwiftSoup 2.13.9, swift-markdown 0.8.0 and licensed emojilib 2.4.0 / emoticon 4.1.0 resources are approved and available. Renderer branch rebased onto main `33f6f1cbc`; both emoji and highlight resources are retained. Parser products remain test-only until integration. All 214 Chat tests pass after rebase (log: /tmp/apple-renderer-rebase-tests.log).
- Markdown tree integration: the approved Markdown product now constructs the existing native block model directly, preserving empty code/table nodes and task checkboxes. Existing inline formatting remains; parser licenses are bundled. All 215 Chat tests and changed-file lint pass. Xcode/iOS validation and full HTML/emoji integration remain outstanding.
- Emoji integration: Swift converts Markdown text nodes using the approved JSON, in shortcode/emoticon order with JavaScript whitespace boundaries. Installed web plugin probes confirm partial emoticon matches in unknown shortcodes and literal task-marker text. Code and link destinations stay unchanged. All 219 Chat tests and changed-file lint pass. Xcode app build/tests passed (/tmp/apple-renderer-parser-app.log).
- HTML policy checkpoint: explicit SwiftSoup allowlists cover tags, attributes, classes, non-text removal and URL schemes; fixtures cover encoded controls, playback attributes and preserved code text nodes. SwiftSoup license bundled. All 223 Chat tests and changed-file lint pass. Policy is connected to native inline and block rendering. Native attributed runs remain protected during HTML parsing; headings, paragraphs and nested lists retain structure. All 225 Chat tests, changed-file lint and iOS 17 compilation pass. Updated Xcode app build/tests passed (/tmp/apple-html-app.log). Remaining HTML/visual parity still requires audit.
- Loose Markdown normalization now matches the web helper, including its pre-parse code-fence behavior. Exact web-function probes and regression fixtures agree; all 229 Chat tests pass.
- Signed visual verification (2026-09-10): current build succeeds with Apple Development signing. Opened the existing Noodles report thread; screenshots confirm headings, bold text, nested lists, clipped body, Show more expansion through section 7 and a visible task link. Collapsed and returned to Everyone without sending. Light mode, text selection/copy and live incremental rendering still need verification.
- Supported-language alias audit: added web aliases jsp, ipython, obj-c++ and objective-c++; existing Java/Python/Objective-C parsers are reused. All 11 highlighter tests and changed-file lint pass. GraphQL/LESS were subsequently approved and merged in #4382.
- Registry audit against installed lowlight/common: remaining unsupported entries are arduino, php-template, python-repl, shell, vbnet and wasm (plaintext needs no parser). Existing native cases also include HTML/TOML/TSX. Detection remains incomplete. Do not substitute adjacent grammars without validating behavior or claim registry parity.
- Markdown image fallback: labels retain safe destinations through the existing link policy; empty labels show the destination. Regression reproduced lost links before the fix; all 12 Markdown tests and changed-file lint pass. This is text/link preservation, not attachment preview completion. Raw HTML image labels and safe source links are also preserved in inline/block HTML, with decoded entities and unsafe-source regression coverage; all 7 inline HTML tests and changed-file lint pass. Audio/video fallback and image previews remain later work.
- Next: complete ordinary HTML/text rendering and visual/interaction checks. Syntax refinements are deferred to 10b by the user. Markdown tree replacement and emoji conversion are integrated; no renderer PR yet.
