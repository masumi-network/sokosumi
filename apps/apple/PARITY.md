...
- Thread-preview parity (24a) merged in [#4629](https://github.com/masumi-network/sokosumi/pull/4629). Final head `b181163ee351606cc20704227cf34f87553fe7cd` passed all CI checks, including macOS build/app tests, all five package suites and strict lint/format. Slice 25 is implemented in draft [#4630](https://github.com/masumi-network/sokosumi/pull/4630), branch `codex/apple-message-deep-links`, based on fetched `origin/main`. Local checks pass; CI and human review/merge remain. Existing pin/search context loaders are reuse candidates; external HTTPS-to-app routing is deferred by user approval; this slice handles links clicked inside Apple.
...
- Current PR: in-app message/thread deep links (25), [#4630](https://github.com/masumi-network/sokosumi/pull/4630); thread-preview parity (24a) and room-scoped thread overview (24) are merged. Quick reactions (20b), room search (23), and shared read-budget cooldown (07b) are merged.
...
| 25 | In-app message/thread deep links and quote/search targets: fetch surrounding history, select room/parent, highlight target and handle missing/forbidden targets. | 08, 23 | In review — [#4630](https://github.com/masumi-network/sokosumi/pull/4630), in-app first approved | [use-message-param-jump.ts](<../web/src/app/(app)/chat/hooks/use-message-param-jump.ts>), [use-room-notification-deep-link.ts](<../web/src/app/(app)/chat/hooks/use-room-notification-deep-link.ts>), [use-room-message-jumps.ts](<../web/src/app/(app)/chat/hooks/use-room-message-jumps.ts>) |
