...
- Current slice: conversation actions (26), no PR yet; in-app links (25) merged in [#4630](https://github.com/masumi-network/sokosumi/pull/4630); thread-preview parity (24a) and room-scoped thread overview (24) are merged. Quick reactions (20b), room search (23), and shared read-budget cooldown (07b) are merged.
...
| 26 | Conversation actions: pin/unpin a conversation in the sidebar (web label Pin, Core star routes; message pins are row 21), mute/unmute, mark unread with optimistic rollback and sidebar ordering/attention. | 05, 07 | In progress — `codex/apple-conversation-actions` | [chat-room-sidebar-row.tsx](<../web/src/components/chat/chat-room-sidebar-row.tsx>), [organization-chat-list.actions.ts](<../web/src/components/chat/organization-chat-list.actions.ts>) |
