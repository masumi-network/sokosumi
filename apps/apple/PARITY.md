...
- Current investigation: message/thread deep links (25); thread-preview parity (24a) and room-scoped thread overview (24) are merged. Quick reactions (20b), room search (23), and shared read-budget cooldown (07b) are merged.
...
| 25 | In-app message/thread deep links and quote/search targets: fetch surrounding history, select room/parent, highlight target and handle missing/forbidden targets. | 08, 23 | In progress — in-app first approved | [use-message-param-jump.ts](<../web/src/app/(app)/chat/hooks/use-message-param-jump.ts>), [use-room-notification-deep-link.ts](<../web/src/app/(app)/chat/hooks/use-room-notification-deep-link.ts>), [use-room-message-jumps.ts](<../web/src/app/(app)/chat/hooks/use-room-message-jumps.ts>) |
