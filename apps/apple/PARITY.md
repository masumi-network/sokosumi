...
- Next: finish replacing Foundation block reconstruction with the full Markdown tree, apply the web HTML policy through SwiftSoup, and convert emoji text nodes in Swift. Preserve the current native views, scrolling improvements, code protection and URL policy. Finish rendering verification before opening the slice PR.
