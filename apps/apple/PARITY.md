...
- Active slice: shared chat read-budget cooldown (07b), branch `codex/apple-chat-read-cooldown`, based on merged main `ee2b19688`. Implementation and local verification are complete in [#4614](https://github.com/masumi-network/sokosumi/pull/4614). Initial CI passed. Review follow-ups address long-wait jitter, its regression coverage, the static operation allowlist, and this checkpoint. Next: verify follow-up CI and wait for human merge before another slice. The existing client middleware stack now shares an account-generation-scoped cooldown across room/thread/search/message/pin/sidebar reads, preserving retry timing and cancelling obsolete reads. Realtime and writes remain independent. See slice 07b below.
...
- Next implementation after #4614 merges: quick reactions (20b). Shared read-budget follow-up (07b) is the active PR, not the next slice. Room search (23) and the documentation refresh (#4608) are merged.
...
| 07b | Honor shared chat read-budget backoff across HTTP recovery readers: retain Retry-After/body delay, coordinate cooldown, bounded fallback and delayed resume; keep realtime events live and isolate account lifetime. | 07, 08 | In review — [#4614](https://github.com/masumi-network/sokosumi/pull/4614) | [chat-read-throttle.ts](<../web/src/lib/chat/chat-read-throttle.ts>), [use-chat-refresh-scheduler.ts](<../web/src/components/chat/use-chat-refresh-scheduler.ts>) |
