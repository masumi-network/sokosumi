...
- Long-message expansion is implemented using 16 measured native body lines. Xcode build/tests and lint pass; signed-app accessibility actions now verify expand/collapse for two existing reports with independent state. Visual clipping, pointer interaction, selection and resizing remain unverified; reports are outside the visible viewport and prior CUA scroll input failed (`noWindowsAvailable`).
