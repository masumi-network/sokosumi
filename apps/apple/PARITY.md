...
- HTML policy checkpoint: explicit SwiftSoup allowlists cover tags, attributes, classes, non-text removal and URL schemes; fixtures cover encoded controls, playback attributes and preserved code text nodes. SwiftSoup license bundled. All 223 Chat tests and changed-file lint pass. Policy is not yet connected to message rendering; next integrate its DOM while preserving Markdown inline styles and block structure.
