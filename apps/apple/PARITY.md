...
- Active slice: thread overview (24), [draft #4626](https://github.com/masumi-network/sokosumi/pull/4626), branch `codex/apple-thread-overview`, rebased on `origin/main` at `29eee6c67`. Room scope matches the user's approval. Existing list/count/mark-all/preferences bindings, portable state, inspector and native navigation are implemented. All five package suites, macOS build/app tests, strict lint/format and iOS 17 Workspace compilation pass locally. Nine overview tests include parameterized preview/preference/stale-response cases. Native fixtures verify narrow light/dark layout and open/back navigation, with limitations below. Next: CI and review, then human merge.
