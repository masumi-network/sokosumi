...
- Active slice: automatic older-reply pagination on `codex/apple-thread-pagination`, based on merged [#4576](https://github.com/masumi-network/sokosumi/pull/4576). Reuses the existing thread loader and room scroll-intent guard when the first loaded reply becomes visible during user scrolling. Initial layout does not drain pages; failures require explicit retry. The native lazy stack tracks a visible reply, and the loading indicator keeps the boundary height stable until prepared replies arrive.
- Scroll correction follow-up (2026-09-15): a focused native test reproduced content-growth jumps on Xcode 27. A short scroll could end before the geometry callback, leaving follow-latest enabled. Room and thread views now recognize bottom-boundary transitions at unchanged layout size and restrict bottom correction to layout-size changes. All 65 app tests (101 parameterized cases), pinned format, and strict lint pass locally after the fix; updated Xcode 27 CI remains required.
- Verification: 39 paging/thread/read-attention model tests and all 65 app tests passed. The focused pagination fixture also passes with a controlled HTTP response: success/failure, stopping/continuing the gesture, no duplicate requests, and visible text remaining within two backing pixels after insertion. Rendered before/after fixtures were inspected. Pinned lint has zero violations. Local review and pinned format checks are complete; PR CI and human merge are required before the next slice. Room search (23) follows this slice after human merge.
- Stable image loading [#4576](https://github.com/masumi-network/sokosumi/pull/4576) merged on 2026-09-14 with CI green. The standard lazy transcript remains in use; see [image loading follow-up](#image-loading-follow-up).
...
- Pinned-message prerequisite: approved [Core PR #4546](https://github.com/masumi-network/sokosumi/pull/4546) merged on 2026-09-14. The source schema reuses message fields as a nullable object, preserving ordinary non-null messages and the JSON contract. The regenerated Apple client retains populated and unavailable previews; generated output was not hand-edited.
- Current pin verification: 351 Chat tests, 68 Workspace tests, the complete macOS app tests (including room/thread scrolling), signed macOS build, shared Workspace iOS 17 compilation, pinned SwiftFormat and strict SwiftLint pass. First-load regression reproduced a cancelled prefetch leaving an empty panel; the shared model-owned request passes that test. History now follows web's contiguous range/30-row jump-window model, retains recent and live messages, joins overlapping windows, and exposes gaps without grouping messages across them. Live read-only verification confirmed pins populate on first opening, an unloaded older pin becomes visible and highlighted, the inspector closes without losing the target during width reflow, and Latest messages returns to the recent conversation. Earlier visibility-tracking and inspector-resize attempts failed these checks and were corrected before verification. Fixture screenshots in `docs/screenshots/pinned-messages-{light,dark}.png` cover short/long/unavailable previews. Existing generator/AppIntents/cmark warnings remain. No live messages or pins were changed; live pin/unpin mutation remains manual.
- Reactions [#4530](https://github.com/masumi-network/sokosumi/pull/4530) merged. User accepted the More hover/padding improvement on 2026-09-14. Latest local app tests, signed build and lint passed; final remote CI has not been recorded here.
- Message actions use React → Reply → Quote → More; Edit and Delete live in More with Delete last, separated. Existing action permissions and deletion confirmation remain in place. Toolbar labels share scaled icon frames, callout text, consistent spacing and individual hover highlights; More retains a native menu with a square trigger and control-level hover background so native label rendering cannot discard it. App tests and pinned lint/format passed.
- Reaction picker follow-up: moved the popover presenter from the whole message row onto React and keep the toolbar visible while open. Native grouped grid now provides category tabs, global search, suggested/frequently used emoji and hover labels from the existing bundled catalog. Live accessibility checks confirmed opening, category switching, search and Escape dismissal; no live reaction was sent. Verification: 343 Chat tests, native app tests, signed macOS build, iOS 17 Workspace build and pinned lint/format passed.
...
- Drive verification: 313 Chat tests, Xcode app tests, signed macOS build, and shared Workspace iOS 17 compilation passed. Fixtures omit pagination.total and decode files/folders across pages. Native menu and sheet open; the initial live check exposed #4432. Corrected-build live checks passed: listing, full-row folder clicks, nested browsing, breadcrumb return, prefix search and no-results/empty-folder states. Sampled folders contained no selectable files, so live file selection and sending remain unconfirmed. Composer stayed unchanged; nothing was sent. Apple CI passed at `bbe7cd2d6`; the subsequent row hit-target fix passed signed build and pinned lint.
...
- Rich composing #4388 and mentions #4392 are merged. Their implementation and verification history remain below. The separate macOS Emoji & Symbols picker check remains pending; the computer-use CLI could not resolve its Orca app path during the 2026-09-11 check. This is not evidence of an app bug and does not block independent attachment work.
...
- Maintenance [PR #4385](https://github.com/masumi-network/sokosumi/pull/4385) merged as `e71173878`; all Apple CI jobs passed. Sources are grouped by feature and the coordinator lives in `SokosumiWorkspace`. The following emoji [PR #4386](https://github.com/masumi-network/sokosumi/pull/4386) merged as `04ae93319` on 2026-09-10. That iteration used `codex/apple-chat-next-slice`; that branch was followed by the now-merged `codex/apple-chat-mentions`.
...
- Follow-up implemented: human tokens omit slugs; picker subtitles/search use email; old tokens remain readable and preserve their wire text when editing. Web composer serialization, Apple draft restoration, and shared notification previews accept ID-only tokens. Cover creation, draft restoration, copy/edit round trips, recipient resolution and previews, including old tokens and renamed/empty-name users. Coworker/Soko Bot/@all writers remain unchanged. Verification: 291 Chat tests, 55 Workspace tests, Xcode app build/tests, 148 targeted web tests, 141 preview tests, web typecheck and pinned Swift lint/format passed. Full web run: 28 failures in task-activity/time-ago; all 28 reproduce on unchanged main. Native signed-build verification passed: email-domain search, name/email row alignment, and Down + Tab confirmation inserted the selected chip. The composer was restored empty without sending. Web visual verification and live sending remain manual. Review commit `e4147a380` also fixes Core fallback recipient resolution for ID-only tokens when explicit IDs are absent; all 85 Core helper tests and 34 web mention utility tests passed. CI for that commit has passed all test suites; the Xcode build is still running (Apple run `34578262174`).
...
PR #4382 merged as `b29e720a0dc1eabc7c8857cc0229f8e69bc8b234`. Both grammars were explicitly approved on 2026-09-10. Dependency verification passed 247 package tests, Xcode app tests, iOS 17 compilation and lint. See [provenance](docs/graphql-less-grammars.md). Renderer branch rebased onto that merge. Both grammars now integrate with the existing native highlighter (33 language choices), and app-host tests load their bundled queries. All 232 Chat tests passed; Xcode app tests and iOS 17 compilation passed. Final lint cleanup passed strict SwiftLint, SwiftFormat and all 12 highlighter tests. Logs: `/tmp/apple-graphql-less-renderer.log`, `/tmp/apple-graphql-less-renderer-xcode.log`, `/tmp/apple-graphql-less-renderer-ios.log`, `/tmp/apple-graphql-less-renderer-final.log`. The user’s Xcode project ordering edit was restored and remains uncommitted.
