...
- Active slice: thread overview (24), [draft #4626](https://github.com/masumi-network/sokosumi/pull/4626), branch `codex/apple-thread-overview`, rebased on `origin/main` at `29eee6c67`. Room scope matches the user's approval. Existing list/count/mark-all/preferences bindings, portable state, inspector and native navigation are implemented. All five package suites, macOS build/app tests, strict lint/format and iOS 17 Workspace compilation pass locally. Nine overview tests include parameterized preview/preference/stale-response cases. Native fixtures verify narrow light/dark layout and open/back navigation, with limitations below. CI passed on `322e167d3d10aebc31a7185b797f3b5f687ca98f`: macOS build, app tests, all five Swift package suites, strict lint/format and repository checks. Grok review follow-up matches Threads VoiceOver to the `99+` cap and adds lasting inspector/Back coverage. Next: wait for human merge before slice 25.
...
- Current implementation: room-scoped thread overview (24), approved to match current web. Quick reactions (20b), room search (23), and shared read-budget cooldown (07b) are merged.
