...
- Active slice: stable attachment/link-preview loading on `codex/apple-stable-chat-images`, based on merged [#4573](https://github.com/masumi-network/sokosumi/pull/4573) (2026-09-14). Keep the standard `ScrollView`/`LazyVStack`, existing native scroll positioning and background Markdown preparation. Reuse the thumbnail decoder for transcript media and remember image proportions across row remounts; no custom virtualization, row-height cache, prefetch scheduler or new dependency. See [image loading follow-up](#image-loading-follow-up).
- Next authorized follow-up after human merge: automatic older-reply pagination in thread transcripts. Reuse the room's user-scroll/loading/error guards, preserve the reading position and retain manual retry. Initial position, composer inset and new-message follow behavior stay unchanged. Do not adopt SwiftyChat's unconditional scroll-to-bottom on every message-count change. Room search (23) follows these agreed fixes.
