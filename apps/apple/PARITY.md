...
- Follow-up integration audit: web `mention-parser.ts` already reads bare IDs, but `mention-textarea-utils.ts` / `composer-markdown-dom.ts` serialize chip slugs; Apple `ComposerReferenceText.presenting` requires a colon; shared `packages/utils/src/chat-message-preview.ts` requires a colon for preview lookup. Cover creation, draft restoration, copy/edit round trips, recipient resolution and previews, including old tokens and renamed/empty-name users. Keep coworker/Soko Bot/@all formats unchanged.
