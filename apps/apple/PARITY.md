...
Review of #4626 identified a preview parity gap: web’s `formatUnreadThreadsPreview` delegates to `buildChatMessagePreview`, so that helper is not banner-only. Slice 24a below resolves the documented sanitizer differences and supersedes this historical finding; it awaits human merge.
