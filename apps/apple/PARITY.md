...
- Next follow-up after image previews: move sending/sent indicators beside the sender name, or into the left gutter for grouped follow-up messages, matching the web. Keep this separate from attachment rendering.
- Active slice: attachment rendering (15), draft PR [#4425](https://github.com/masumi-network/sokosumi/pull/4425), branch `codex/apple-chat-attachment-previews`, based on main `b94d66fd5e`. Reuses parsed Markdown/HTML links and native SwiftUI/AVKit presentation. No dependencies, Core contracts, or web code change. Drive selection (16) remains separate.
...
| 15 | Attachment rendering: file chips and metadata, image previews, open/download, inline audio/video and safe links; maintain content order with text. | 14 | In review — #4425; manual acceptance pending | [room-message-row.tsx](<../web/src/app/(app)/chat/components/room-message-row.tsx>), [room-message-segments.ts](<../web/src/app/(app)/chat/utils/room-message-segments.ts>), [markdown.tsx](<../web/src/components/markdown.tsx>), [file-chip-mini-preview.tsx](<../web/src/components/ui/file-chip-mini-preview.tsx>) |
