...
- Markdown image fallback: labels retain safe destinations through the existing link policy; empty labels show the destination. Regression reproduced lost links before the fix; all 12 Markdown tests and changed-file lint pass. This is text/link preservation, not attachment preview completion. Raw HTML media fallback remains unaudited.
