...
| 07b | Honor shared chat read-budget backoff across HTTP recovery readers: retain Retry-After/body delay, coordinate cooldown, bounded fallback and delayed resume; keep realtime events live and isolate account lifetime. | 07, 08 | Implemented — local verification complete | [chat-read-throttle.ts](<../web/src/lib/chat/chat-read-throttle.ts>), [use-chat-refresh-scheduler.ts](<../web/src/components/chat/use-chat-refresh-scheduler.ts>) |
