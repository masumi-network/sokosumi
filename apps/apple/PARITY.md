...
- Current slice: Start Direct (27), implemented in draft [#4665](https://github.com/masumi-network/sokosumi/pull/4665) on `codex/apple-start-direct`; local verification passes, awaiting CI/review and human merge; conversation actions (26) merged in [#4640](https://github.com/masumi-network/sokosumi/pull/4640); in-app links (25) merged in [#4630](https://github.com/masumi-network/sokosumi/pull/4630); thread-preview parity (24a) and room-scoped thread overview (24) are merged. Quick reactions (20b), room search (23), and shared read-budget cooldown (07b) are merged.
- Approved follow-up after Start Direct: apply Xcode’s recommended project settings in a separate maintenance PR (27a): enable dead-code stripping, inherit the development team from project settings, and enable String Catalog symbol generation at project level. Preserve CI’s ad-hoc signing overrides and validate Debug/Release settings, macOS build/tests, and iOS package compatibility.
