...
- Active slice: quick reactions (20b), [#4620](https://github.com/masumi-network/sokosumi/pull/4620), branch `codex/apple-quick-reactions`, based on merged main `2a7a20bf4`. Reuse the existing history and reaction action for three toolbar shortcuts; preserve order during interaction and fit narrow thread panes. All five package suites and app tests passed locally. Next: check CI/review, then wait for human merge.
...
- Current implementation: quick reactions (20b). Select the next remaining row only after this slice merges. Room search (23), the documentation refresh (#4608), and shared read-budget cooldown (07b) are merged.
...
| 20b | One-tap reactions before React in the message toolbar; reuse frequent history with default fallback, stable choices while interacting, existing availability/own-reaction behavior, keyboard labels and hover feedback. | 20 | In review — [#4620](https://github.com/masumi-network/sokosumi/pull/4620) | [quick-reactions.ts](<../web/src/app/(app)/chat/utils/quick-reactions.ts>), [room-message-row.tsx](<../web/src/app/(app)/chat/components/room-message-row.tsx>) |
