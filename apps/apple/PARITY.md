...
- Slice 10 is implemented on `codex/apple-rich-message-rendering`, rebased on main `717b73d81`. [PR #4384](https://github.com/masumi-network/sokosumi/pull/4384) is open; keep the row Partial until remaining interaction checks and review are complete. Earlier checkpoints below are historical.
...
- Current verification: 296 package tests (Chat 234, Auth 34, Realtime 27, CoreAPI 1), Xcode app build/tests, iOS 17 compilation of the Chat test target, full Apple SwiftFormat and strict SwiftLint passed. The Realtime lockfile was regenerated by SwiftPM and matches the app’s resolved package revisions.
...
- Reviewed the author's `7757557e6` media fallback: audio/video sources now remain safe linked text, including nested source elements; native players remain slice 15.
- Addressed minor findings 1–4: removed unused HTML token metadata, shared the rendered-link scheme allowlist, retained audio width/height like web, and limited code captions to the first fence token.
- Track remaining non-blocking findings with their owning slices: mark/search emphasis in 23; mention span styling in 13; internal chat link routing in 25; streaming parse coalescing only if runtime verification/profiling identifies jank. Revalidate the pinned SCSS query adjustment whenever TreeSitterLanguages changes (10b). Native view interactions still require the manual checks above; parser tests do not replace them.
