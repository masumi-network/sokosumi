...
Native Sokosumi on Apple. One Xcode workspace, two app targets, shared Swift modules. The first ship is **macOS chat**. **iOS** is the same codebase, later — not a second repo and not `apps/ios`.
...
| Shared Swift modules | `SokosumiKit` (working name) | API client, session, rooms, Ably, unread. Not “Core” (that is the HTTP API). |
...
iOS will want the same session, rooms, and Ably client. Those live in Swift modules the Mac app already uses. Each OS still gets its own UI (Mac windows/menus; iOS navigation/keyboard). One SwiftUI tree with `#if os` is not the plan.
...
The iOS target links `SokosumiKit` and ships its own SwiftUI. It does not fork the Mac UI. Composer, keyboard, and APNs are iOS work. They wait until Mac chat round-trips against Core.
