...
- **Snapshot extraction:** From `apps/apple`, run `python3 scripts/update-core-api.py /path/to/core-openapi.json /chats/rooms/{id}/unread` to retain the selected operations and add a path from a generated Core specification. The script resolves transitive schemas and writes UTF-8; omit additional paths to refresh the existing slice. Do not edit the extracted JSON by hand.
