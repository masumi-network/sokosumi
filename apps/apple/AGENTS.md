...
- `.github/workflows/apple.yml` runs three Apple gates on `xcode-27`: `Xcode test` (the workspace scheme), `Xcode release build` (Release compile) and `Swift lint and format` (SwiftLint + SwiftFormat via Mint). All three share one `if:` gate, run in parallel, and are **required status checks** on `main`, so do not delete or rename any of them without updating the `Default Branch` ruleset in the same change. They run for PRs touching `apps/apple/**` (or manual dispatch), including drafts, and on path-filtered pushes to `main`, which save the Mint binary cache; PRs only restore it.
- `Xcode release build` compiles `-configuration Release`. Nothing else does until a release is cut: `Xcode test` builds Debug, and Xcode Cloud builds Release on the archive that ships, so a whole-module-optimization, `@inlinable` access or dead-stripping break would surface at release. It runs `build`, not `archive` — CI holds no certificates and Xcode Cloud performs the real signed archive.
