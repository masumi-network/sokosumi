...
- **The split between `Xcode release build` and `Publish rolling DMG` is a security boundary. Do not merge them.** Both compile Release, so collapsing them looks like an obvious cleanup — it is not. A pull request from a branch in this repository *does* receive repository secrets, and `xcodebuild` runs contributor-controlled build scripts and SwiftPM plugins. One job that both runs on pull requests and holds the Developer ID key would let an unreviewed pull request exfiltrate the identity that signs the published app. Every step touching a certificate or notary key belongs in `publish`, which only runs after merge (or by manual dispatch). Keep `Xcode release build` free of `secrets.*`.
- `Publish rolling DMG` runs on **main pushes only**, after `Xcode test` and `Swift lint and format` are green. It archives with the Developer ID certificate, exports via [`ExportOptions.plist`](ExportOptions.plist), builds a disk image with `hdiutil`, notarizes it with `notarytool --wait`, staples the ticket, and replaces the single `apple-latest` prerelease. The download URL is therefore permanent:
