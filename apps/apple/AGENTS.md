...
- `Xcode release build` compiles `-configuration Release` on **main pushes only**, never on a pull request, so it is not a required status check. It exists because nothing else compiles Release until a release is cut: `Xcode test` builds Debug, and Xcode Cloud builds Release on the archive that ships. It runs `build`, not `archive` — CI holds no certificates and Xcode Cloud performs the real signed archive.
