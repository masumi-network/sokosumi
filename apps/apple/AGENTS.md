...
- `Publish macOS DMG` runs on **main pushes only**, after `Xcode test` is green (not `Swift lint and format` — style cannot make a binary broken, so a working notarized download is not withheld over indentation). It archives with the Developer ID certificate, exports via [`ExportOptions.plist`](ExportOptions.plist), builds a disk image with `hdiutil`, notarizes it with `notarytool --wait`, staples the ticket, and replaces the single `apple-latest` prerelease. The download URL is therefore permanent:
