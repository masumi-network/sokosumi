...
- `Publish macOS DMG` runs on **main pushes only**, after `Xcode test` and `Swift lint and format` are green. It archives with the Developer ID certificate, exports via [`ExportOptions.plist`](ExportOptions.plist), builds a disk image with `hdiutil`, notarizes it with `notarytool --wait`, staples the ticket, and replaces the single `apple-latest` prerelease. The download URL is therefore permanent:
