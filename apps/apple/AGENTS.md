...
- `Xcode release build` compiles `-configuration Release` on **pull requests and dispatch only**, catching a whole-module-optimization, `@inlinable` access or dead-stripping break that Debug accepts. It runs unsigned `build`. Deliberately not on `push`: `Publish rolling DMG` archives Release on the merge commit, and compiling Release twice per Apple merge is waste.
