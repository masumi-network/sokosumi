...
Verification: the full workspace suite passes **867 tests, zero failures/skips**, before and after the product change. This includes the 16 existing content-growth/reading-position cases, four room/thread media-scrolling cases, and the expanded four light/dark room/thread jump cases that request navigation before preparation finishes. Release harness builds and the shared Workspace build targeting iOS 17 pass. Strict uncached SwiftLint and pinned SwiftFormat pass. At measured revision `4cd2f1a7da1160e9294443c4014f62440aa6560c`, the inlined sources recreate the measured app/probes byte for byte, with the Xcode project and package-product linkage unchanged. Subsequent review cleanup removes two comments from the room view; the retained hashes still identify the measured sources. An app-only rerun passes 100 tests after making the snapshot fixture background explicit. All four light/dark room/thread captures were inspected: the requested message is centered and highlighted, with the composer and Jump to latest control visible. The existing large room view composition has a scoped length/complexity lint annotation after becoming a parameterized function; its branches and view tree were not expanded.

The review follow-up strengthens those four jump cases: after mounting, room navigation must be consumed while the thread must retain its requested jump target. Apple's Vision framework recognizes the existing captured bitmap and requires `Message 2:` to be visible and distant `Message 98:` to be absent. All four cases pass. A temporary control requests message 50 while still expecting message 2; all four cases fail only the new rendered-target assertion, even though the state and distance-from-bottom checks pass. The control is reverted. Highlight appearance remains covered by the manual snapshot inspection above; the test adds no package products or project linkage.
