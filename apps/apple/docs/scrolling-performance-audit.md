...
### Prepare transcript snapshots before displaying rows

Added portable `PreparedTranscript` to SokosumiChat. Room and thread views prepare message/document snapshots in a detached task before publishing rows. Unchanged documents are reused only within the same scope and resolution context; the cache is bounded to the previous snapshot. Message content and parsed blocks publish together, so edits/streams retain a consistent previous rendering until the next snapshot is ready. Cancellation propagates into preparation and is checked again before publication. Room/account/thread/generation changes hide the previous snapshot. Pins retain the standalone asynchronous renderer and its native loading placeholder.

The final serial fixture (PID 51971, `/tmp/scroll-prepared-final-scrolling.log`) passes all seven parameterized scrolling/content-growth cases. Layout p95: room text 0.084 ms, thread text 0.100 ms, room media 0.063 ms, thread media 0.718 ms. A first prepared-snapshot run (PID 50013) measured 0.242/0.175/1.948/1.851 ms respectively. The preceding placeholder-only run measured 24.130/14.561/13.287/10.985 ms. These measure synchronous host layout, not display frame rate; media step p95 still includes about 36 ms of event, layout and scheduling time. Live smoothness still requires user confirmation.
