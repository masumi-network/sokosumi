...
Modified Return inserts a line break within the current block, including a list item. On an empty final quote line it exits the quote, matching the web editor. The Code block control is a toggle: `ComposerBlockFormat.edit(in:selection:)` reads the whole text, so with the caret in a block it unwraps that block into plain lines, and otherwise it wraps the selection or opens an empty block that holds the caret. Formatting controls refresh when typing attributes change, even before the first character is inserted. Unsupported restored structures retain their original text instead of being dropped.
