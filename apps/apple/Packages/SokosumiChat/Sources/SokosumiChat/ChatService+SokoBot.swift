...
public extension ChatService {
  ...
  /// rating of one Soko Bot turn (web `sendSokoBotTurnFeedbackAction`). Turns
  /// are user-scoped, so the operation carries no organization header.
  /// 404 = not one of the caller's turns. Returns the stored value.
