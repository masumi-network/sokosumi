...
public extension ChatService {
  func toggleReaction(client: Client, roomId: String, messageId: String, emoji: String, organizationSlug: String?) async throws -> Components.Schemas.ChatRoomMessage {
    let response = try await client.postChatsRoomsIdMessagesMessageIdReactions(.init(
      path: .init(id: roomId, messageId: messageId),
      headers: .init(xOrganizationSlug: organizationSlug),
      body: .json(.init(emoji: emoji))
    ))
    switch response {
