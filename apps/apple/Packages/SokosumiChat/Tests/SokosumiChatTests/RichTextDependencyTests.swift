...
struct RichTextDependencyTests {
  @Test func preservesEmptyCodeBetweenParagraphs() throws {
    let document = Document(parsing: "before\n\n```swift\n```\n\nafter")
  ...
  @Test func preservesEmptyTableRowsAndCells() throws {
    for source in [
      ...
      let table = try #require(Array(Document(parsing: source).children).first as? Markdown.Table)
  ...
  @Test func loadsApprovedEmojiResources() throws {
    ...
    let shortcodes = try JSONDecoder().decode([String: String].self, from: Data(contentsOf: shortcodesURL))
    ...
    let emoticons = try JSONDecoder().decode([Emoticon].self, from: Data(contentsOf: emoticonsURL))
