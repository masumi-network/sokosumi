...
describe("requireUserTaskAccess", () => {
  ...
  it("uses user ownership only with owned scope", async () => {
    ...
    await requireUserTaskAccess(userAuthContext, "tsk_123", tx, ["owned"]);
...
describe("requireTaskAccess", () => {
  it("keeps coworker access path unchanged when owned scope is requested", async () => {
    const tx = createTransactionClient();
    ...
    await requireTaskAccess(coworkerContext, "tsk_123", tx, ["owned"]);
