 ...
 * damage said more quietly. `null`, a bare number and an array all parse, and
 * none of them carries a field any reader here asks for, so each one leaves
 * the reader on the same fallback a throw does. Reporting the throw alone
 * would leave the quieter half of the same defect silent. An array is checked
 * for by name because `typeof` calls it an object, and a caller asking it for
 * a field would read `undefined` from it rather than be told.
 *
...
/**
 * How many damaged columns one process names before it stops naming them.
 *
 * Columns rather than rows, because a row damaged in both of them has two
 * things to say and says both. So this is five hundred rows at its most
 * generous and two hundred and fifty at its least. The number bounds what
 * this set can hold; which rows fill it is the defect's business.
 *
 * Reached only by a defect that damaged rows in bulk, and by then the first
 * few hundred have said everything the next one would.
 */
const REPORTED_COLUMN_LIMIT = 500;
...
function alreadySaid(rowId: string, field: NotificationRowJsonField): boolean {
  ...
  if (reportedRows.size >= REPORTED_COLUMN_LIMIT) {
    return true;
  }
