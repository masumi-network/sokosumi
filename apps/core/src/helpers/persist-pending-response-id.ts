...
export interface ClearCoworkerResponseChainParams {
  conversationId: string;
  userId: string;
}

export interface ClearCoworkerResponseChainOptions {
  maxAttempts?: number;
  delaysMs?: number[];
}

export async function clearCoworkerResponseChain(
  params: ClearCoworkerResponseChainParams,
  options: ClearCoworkerResponseChainOptions = {},
): Promise<void> {
  const { conversationId, userId } = params;
  const maxAttempts = options.maxAttempts ?? 3;
  const delaysMs = options.delaysMs ?? [0, 150, 300];
  ...
  throw (
    lastError ??
    new Error("Failed to clear coworker response chain after retries")
  );
}
