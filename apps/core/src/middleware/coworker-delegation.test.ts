...
describe("hasCoworkerUserDelegation", () => {
  ...
  it("counts a PENDING grant so lazy delegated create is not deadlocked", async () => {
    vendorGrantFindFirstMock.mockResolvedValue({ id: "grant_pending" });

    await expect(hasCoworkerUserDelegation(params)).resolves.toBe(true);
    const where = vendorGrantFindFirstMock.mock.calls[0]?.[0]?.where;
    expect(where.status).toEqual({ notIn: ["DENIED", "REVOKED"] });
  });
