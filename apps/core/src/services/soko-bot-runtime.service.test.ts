...
describe("SokoBotRuntimeService authorization", () => {
  ...
  it("does not return PRIVATE peer links Alice cannot see from readTask", async () => {
    turnFindUniqueMock.mockResolvedValue({
      userMessage: "Check the tasks",
      id: SCOPE.turnId,
      sokoBotId: SCOPE.sokoBotId,
    ...
    const visiblePeerTask = {
      workspaceId: SCOPE.workspaceId,
      archivedAt: null,
      ...buildSokoBotOwnerTaskVisibilityWhere(SCOPE.userId),
    };
    ...
    taskFindFirstMock.mockImplementation(
      async (args: {
        ...
        const hidesPrivatePeers =
          JSON.stringify(args.select?.linksFrom?.where) ===
            JSON.stringify(visiblePeerWhere) &&
          JSON.stringify(args.select?.linksTo?.where) ===
            JSON.stringify(visibleFromPeerWhere);
        return {
          id: "public-1",
          name: "Public launch",
          status: "READY",
    ...
    const result = await new SokoBotRuntimeService().executeTool({
      ...SCOPE,
      capability: "get_task_status",
      toolCallId: "call_owner_task_links",
      ...
      expect.objectContaining({
        select: expect.objectContaining({
          linksFrom: expect.objectContaining({
            where: visiblePeerWhere,
          }),
          linksTo: expect.objectContaining({
            where: visibleFromPeerWhere,
          }),
        }),
      }),
    );
    expect(visiblePeerTask).toEqual({
      workspaceId: SCOPE.workspaceId,
      archivedAt: null,
      OR: [
        { visibility: TaskVisibility.PUBLIC },
        { visibility: TaskVisibility.PRIVATE, ownerId: SCOPE.userId },
      ],
    });
