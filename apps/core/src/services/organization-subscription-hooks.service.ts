...
/**
 * Port of the web app's `organization-subscription.service.ts` hook methods
 * (the seats write already lives in core, #3114). These run inside the Better
 * Auth organization plugin hooks: `ensureCanAcceptInvitation` on
 * beforeAcceptInvitation, `syncLocalFreeSeatsAndCreditsForCurrentMembers` on
 * afterAcceptInvitation/afterAddMember. APIError messages are byte-identical
 * to web's so client error handling is unchanged.
 */
...
export const organizationSubscriptionHooksService = (() => {
  return {
