...
export default function mount(app: OpenAPIHonoWithAuth) {
  app.openapi(route, async (c) => {
    ...
    const { live, memberUserIds } = await prisma.$transaction(async (tx) => {
    ...
    // After commit: sidebar flags Promise.all on the default client (#2559).
    const restored = await mapChatRoomWithSidebarFlags(
      live,
      userContext.userId,
      prisma,
