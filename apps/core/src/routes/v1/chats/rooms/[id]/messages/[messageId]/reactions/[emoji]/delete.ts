...
const route = withOrganizationSlugHeaderParameter(
  createRoute({
    ...
    description:
      "Remove the current user's emoji Reaction from a room message. Idempotent if it does not exist.",
    tags: ["Chat Rooms"],
    request: {
      params: chatRoomMessageReactionParamsSchema,
    },
    responses: {
      200: jsonSuccessResponse(chatRoomMessageSchema, "Reaction removed"),
