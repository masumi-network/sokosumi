 ...
 * Stable order: user/coworker/orchestrator left, then the same types joined.
 */
export function diffChannelMembershipRoster(args: {
  ...
  const nextOrchestratorIds = new Set(
    args.next.orchestrators.map((orchestrator) => orchestrator.id),
  );
  const priorOrchestratorIds = new Set(
    args.prior.orchestrators.map((orchestrator) => orchestrator.id),
  );
