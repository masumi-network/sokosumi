...
// The removal has no body, so its concurrency token travels as a request
// header. It is deliberately NOT the standard `If-Match` precondition: Vercel's
// edge evaluates `If-Match` against the resource and returns `412
// PRECONDITION_FAILED` before the request ever reaches Core.
const SCHEDULE_REVISION_PATTERN = /^(0|[1-9]\d*)$/;
...
// canonical headers in their matched-case form.
const headersSchema = z.object({
  ...
  "x-schedule-revision": z
    .string()
    .regex(
      SCHEDULE_REVISION_PATTERN,
      "x-schedule-revision must be a non-negative integer",
    )
    .openapi({
      param: { in: "header" },
...
export default function mount(app: OpenAPIHonoWithAuth) {
  app.openapi(route, async (c) => {
    ...
    const {
      ...
      "x-schedule-revision": scheduleRevisionHeader,
    } = c.req.valid("header");
    const expectedScheduleRevision = Number(scheduleRevisionHeader);
