...
const route = createRoute({
  method: "post",
  path: "/{grantId}/approve",
  description:
    "Approve a vendor access grant for the authenticated session user (path must be `me`).",
  tags: ["Users"],
  request: {
    params,
  },
  ...
  app.openapi(route, async (c) => {
    ...
    const { grant, affectedTaskIds } = await serializableTransaction(
      async (tx) => {
        ...
        if (!canApproveGrant(existingGrant.status)) {
          throw unprocessableEntity(
            `Cannot approve vendor access grant with status ${existingGrant.status}`,
          );
        }
      ...
      "Could not approve vendor access grant",
    );
