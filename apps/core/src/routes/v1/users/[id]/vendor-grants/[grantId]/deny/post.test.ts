...
function createApp(authContext: AuthenticationContext = SESSION_USER) {
  ...
  mountDenyUserVendorGrant(userByIdApp);
...
beforeAll(async () => {
  ...
  mountDenyUserVendorGrant = module.default;
});

describe("POST /users/{id}/vendor-grants/{grantId}/deny", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("denies PENDING workspace grant and cancels parked tasks", async () => {
    ...
    const response = await createApp().request(
      `http://localhost/me/vendor-grants/${grantId}/deny`,
      { method: "POST" },
    );
