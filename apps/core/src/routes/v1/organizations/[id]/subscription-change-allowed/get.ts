...
const subscriptionChangeAllowedSchema = z
  ...
  .openapi("SubscriptionChangeAllowed");

const route = createRoute({
  method: "get",
  path: "/{id}/subscription-change-allowed",
  description:
    "Validate that self-serve subscription changes are allowed for the organization (blocks consumable enterprise contracts).",
  tags: ["Organizations"],
  request: {
    params,
  },
  ...
  app.openapi(route, async (c) => {
    ...
    try {
      await prisma.$transaction(async (tx) => {
        ...
        await assertOrganizationSubscriptionChangeAllowed(id, tx);
      });
    } catch (error) {
