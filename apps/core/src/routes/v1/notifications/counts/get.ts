...
export default function mount(app: OpenAPIHonoWithAuth) {
  app.openapi(route, async (c) => {
    ...
    // the number of rows the Needs you view shows and nothing else.
    const [unread, needsAction] = await Promise.all([
    ...
    return ok(c, notificationCountsSchema.parse({ unread, needsAction }));
  });
}
