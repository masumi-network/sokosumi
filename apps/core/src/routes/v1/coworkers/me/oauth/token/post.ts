...
const requestSchema = z.object({
  ...
  scope: z.string().optional().openapi({ example: "openid offline_access" }),
});
...
export default function mount(app: OpenAPIHonoWithAuth) {
  app.openapi(route, async (c) => {
    ...
    const { userId, organizationId, clientId, scope } = c.req.valid("json");
    ...
    const scopes = resolveScopes(scope, oauthClient.scopes);
    if (scopes.length === 0) {
