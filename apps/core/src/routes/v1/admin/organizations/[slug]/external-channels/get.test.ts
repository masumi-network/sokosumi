...
import mountListAdminOrgExternalChannels from "./get";

const {
  getAdminOrganizationBySlugMock,
  listAdminExternalChannelsMock,
  authContextState,
} = vi.hoisted(() => ({
  authContextState: {
    current: {
  ...
  listAdminExternalChannelsMock: vi.fn(),
}));
...
vi.mock("@/helpers/admin-organization-overview.js", () => ({
  ...
  listAdminExternalChannels: (...args: unknown[]) =>
    listAdminExternalChannelsMock(...args),
}));
...
describe("GET /admin/organizations/{slug}/external-channels", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    ...
    listAdminExternalChannelsMock.mockResolvedValue([
      { id: ROOM_ID, name: "Partners", slug: "partners" },
    ]);
  });

  it("lists live external channels for the host org", async () => {
    ...
    expect(listAdminExternalChannelsMock).toHaveBeenCalledWith(
      "org_1",
      expect.anything(),
    );
  });
  ...
  it("returns 404 when the organization does not exist", async () => {
    ...
    expect(listAdminExternalChannelsMock).not.toHaveBeenCalled();
  });
