...
| `ABLY_SUBSCRIBE_ONLY_KEY` | Ably key for minting client TokenRequests (`POST /v1/realtime/ably-token`); must allow subscribe on app channels and `presence`+`subscribe` on `presence:org_*` (ADR-0002) |
