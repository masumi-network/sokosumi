...
| `login` `{provider}`  | signed in. Social and magic-link fire on `/auth/callback/signin` after the full page load. Credential (`signin/components/form.tsx`) and passkey (`social-buttons.tsx`) fire in place before `router.replace` | `components/social-auth-callback.tsx`, `signin/components/form.tsx`, `components/social-buttons.tsx` |
