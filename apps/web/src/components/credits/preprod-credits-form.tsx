...
const preprodCreditsFormSchema = z.object({
  credits: z.number().min(1).max(10000),
});

type PreprodCreditsFormData = z.infer<typeof preprodCreditsFormSchema>;
...
export default function PreprodCreditsForm({
  ...
  const form = useForm<PreprodCreditsFormData>({
    resolver: zodResolver(preprodCreditsFormSchema),
    defaultValues: {
      credits: undefined,
    },
  });
