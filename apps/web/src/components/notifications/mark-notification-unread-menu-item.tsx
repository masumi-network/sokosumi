 ...
 *
 * The way out of a read the reader did not mean. A menu item of its own rather
 * than a button inside the row's item, for the same reason the delete control
 * is one: arrow keys reach it, and each element in the menu keeps one role.
 *
 * The icon is a sealed envelope, the state the control produces, and the
 * tooltip says the same thing in words. An icon alone is a guess.
 */
        ...
        <DropdownMenuItem
          ...
          onSelect={(event) => {
            ...
            markUnread(notificationId).catch(() => {
              // The provider logged it and refetched. The row stays read,
