...
/**
 * `same-origin` (Next.js / hosting default in some setups) breaks Composio
 * OAuth popups: once the popup navigates to connect.composio.dev, COOP puts it
 * in a separate browsing context group, so the opener cannot read
 * `popup.closed` and `window.opener` may be null on the callback page.
 *
 ...
 * reference through a cross-origin redirect — use BroadcastChannel in
 * `lib/composio/oauth-popup-protocol.ts` for callback delivery.
 *
