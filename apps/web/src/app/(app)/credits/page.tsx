...
import CreditsForm from "@/components/credits/credits-form.mainnet";
import PreprodCreditsForm from "@/components/credits/credits-form.preprod";
...
export default async function CreditsPage({ searchParams }: CreditsPageProps) {
  ...
  // Only fetch Stripe data for Mainnet
  const productId = isMainnet ? getEnvSecrets().STRIPE_PRODUCT_ID : null;
  const price = isMainnet
    ? await stripeClient.getPriceByProductId(productId!)
    : null;

  // for credits success modal (Mainnet only)
  const checkoutSessionPromise =
    isMainnet && session_id
    ...
    <div className="w-full space-y-12 px-2">
      ...
      <div className="max-w-3xl">
        {isMainnet && price ? (
          ...
          <PreprodCreditsForm organization={activeOrganization} />
        )}
      </div>
    </div>
  );
