...
const MESSAGES: Record<string, string> = {
  ...
  connectAccountLink: "Open Project settings",
  needsReconnect: "Account needs reconnecting",
  needsReconnectLink: "Reconnect in Project settings",
  viewOnX: "View on X",
...
describe("ProjectSocialPosts", () => {
  ...
  it("explains that an account must be connected and links to Project settings", () => {
    render(
      <ProjectSocialPosts
        connections={[]}
        locale="en"
    ...
    expect(
      within(callout).getByRole("link", { name: "Open Project settings" }),
  ...
  it("warns when the linked account needs reconnecting and links to Project settings", () => {
    render(
      <ProjectSocialPosts
        connections={[buildConnection()]}
        locale="en"
    ...
    expect(
      within(warning).getByRole("link", {
        name: "Reconnect in Project settings",
      }),
    ).toHaveAttribute("href", "/projects/project-1/edit");
  });
