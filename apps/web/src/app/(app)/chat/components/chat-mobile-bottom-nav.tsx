...
export function ChatMobileBottomNav(): React.ReactElement {
  ...
  return (
    <>
      {/* The sidebar sheet is closed on mobile, so its control bridge is
          not mounted; archive, restore, and invitation invalidations still
          have to reach the unread dot (SOK-986). */}
      {sessionUser ? (
        <LazyAblyProvider>
          <ChatControlListBridge currentUserId={sessionUser.id} />
        </LazyAblyProvider>
      ) : null}
      <nav
