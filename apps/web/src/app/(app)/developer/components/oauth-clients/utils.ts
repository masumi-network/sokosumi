...
/**
 * Client-side redirect URI check using Better Auth's SafeUrlSchema
 * (single source of truth with server-side validation).
 */
export function isSafeRedirectUri(uri: string): boolean {
  return SafeUrlSchema.safeParse(uri).success;
}
